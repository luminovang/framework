...
class NovaLogger extends AbstractLogger
{
    ...
    ) 
    {
        ...
        $ext = strtolower(trim($this->extension, " ."));
     ...
     *
     * @return string Return the dirpath.
     */
    ...
    {
        LogLevel::assert($level, __METHOD__, true);

        $this->level = LogLevel::resolve($level) ?? $level;
        return $this;
    }
     ...
     *
     * Supports both predefined and custom log levels.
     * Custom levels must follow validation rules defined in {@see LogLevel::assert()}.
     *
     * @param string|int $level  Log level identifier (e.g. "error", "info", or custom like "test123").
     * @param string $message The log message to record.
     * @param array<string,mixed> $context Optional contextual data for interpolation or debugging.
     *
     * @return void
     *
     * @throws FileException If the log cannot be written to the storage destination.
     * @throws InvalidArgumentException If the provided log level is not valid.
     *
     * @example - Examples:
     * ```php
     * $logger->log('error', 'User login failed', [
     *     'user_id' => 42,
     *     'ip'      => '192.168.1.1'
     * ]);
     *
     * // Custom level
     * $logger->log('audit2026', 'Permission changed', [
     *     'by' => 'admin'
     * ]);
     * ```
     */
    ...
    public function mail(string $email, string $message, array $context = []): void 
    {
        if(!$email || !Validator::isEmail($email)){
            $this->write(
                LogLevel::CRITICAL, 
                sprintf('Invalid mail logger email address: %s', $email), 
                [
        ...
        $entry = self::toMessage($this->level, $message, $this->name, $context) . PHP_EOL;
        $entry .= self::toMessage($this->level, $error, $this->name);
    ...
    public function remote(string $url, string $message, array $context = []): void 
    {
        if(!$url || !Validator::isUrl($url)){
            $this->write(
                LogLevel::CRITICAL, 
                sprintf('Invalid remote logger endpoint URL: %s', $url), 
                [
                    'originalMessage' => $message,
                    'originalContext' => $context,
                    'originalLevel'   => $this->level,
        ...
        if($data === null){
            ...
            if(PRODUCTION){
               $this->write($this->level, $message, $context, resolveLevel: false);
                return;
            }
    ...
    public function telegram(string|int $chatId, string $token, string $message, array $context = []): void 
    {
        if (!$token || !$chatId) {
            $this->write(
                LogLevel::CRITICAL, 
                'Telegram log bot token or chat ID is missing', 
                [
                    'originalMessage' => $message,
                    'originalContext' => $context,
                ],
    ...
    public function slack(string $webhookUrl, string $message, array $context = []): void
    {
        if (!$webhookUrl) {
            $this->write(
                LogLevel::CRITICAL, 
                'Slack log webhook URL is missing', 
                [
                    'originalMessage' => $message,
                    'originalContext' => $context,
                ],
    ...
    public function clear(?string $level = null): bool 
    {
        ...
        if(
            $level === null 
            || (!LogLevel::has($level) && !LogLevel::isValid($level))
        ){
            return false;
        }
    ...
    {
        if(!LogLevel::has($level) && !LogLevel::isValid($level)){
            return null;
        }
        ...
        return $htmlFormat 
            ...
            : self::toMessage($level, $message, $this->name, $context);
    }
    ...
    /**
     * Process and normalize log message template.
     *
     * @param string $level
     ...
     * 
     * 
     * @return string|null Return template.
     */
    private static function toMessage(
        string $level,
        string $message,
        ?string $name = null,
        array $context = [],
    ...
    ): ?string 
    {
        ...
        $replacements = [
            '{level}'   => strtoupper(LogLevel::resolve($level) ?? $level),
            '{message}' => $message,
            '{name}'    => '',
            '{context}' => ($context === [])
                ? ''
    ...
    ): bool
    {
        ...
        if($resolveLevel){
            $lve = LogLevel::resolve($level);
            $level = ($lve === null && LogLevel::isValid($level))
                ? $level 
                : ($lve ?? LogLevel::INFO);
        }
    ...
    /**
     * Logs an exception error with fallback file logging.
     *
     * This is used when primary logging fails and must not throw exceptions.
     *
     * @param string $filename Log file name (context-specific).
     * @param string $error Error message from exception or logger failure.
     * @param string $message Original log message being processed.
     *
     * @return void
     */
    private function e(string $filename, string $error, string $message): void
    {
        $log = sprintf(
            "ORIGINAL: %s%sERROR: %s",
            $message,
            PHP_EOL,
            self::toMessage($this->level, $error, $this->name)
        );

        if (error_log($log, 3, self::$path . $filename) === false && PRODUCTION) {
            error_log($log);
        }
    }
     ...
     *
     * @return string|null Return error message if failed to send log when `$async` is false, otherwise null.
     * @internal Used by Async background process.
     * @ignore
     */
    public static final function __flushRemote(array $args, bool $async = true): ?string 
    {
        ...
        try {
            $response = Luminova::kernel('http.client')
