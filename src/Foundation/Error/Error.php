...
final class Error
{
     ...
     * 
     * @param array<string,mixed>|null $error Optional shutdown error details. 
     * 
     * Behavior:
     ...
     * ```php
     * Error::deprecate('Method %s() is deprecated. Use %s() instead.', '1.5.0', ['foo', 'getFoo']);
     * ```
     */
     ...
     * 
     * Accessible via `Error::getBacktrace()`.  
    ...
    private static function log(string $level, string $message, ?bool $isUp = null): void 
    {
        ...
        if (@file_put_contents($filename, $formatted . PHP_EOL, FILE_APPEND | LOCK_EX) === false) {
            error_log($formatted . PHP_EOL, 3, $filename);
        }
    }
    ...
    private static function display(?Message $error = null): void 
    {
        ...
        $error ??= 'An error has prevented the application from running correctly.';
        Luminova::terminate(500, (string) $error, retry: 60);
    }
}