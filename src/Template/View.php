...
{ 
    ...
    public function __construct(Application|LazyObjectInterface|null $app = null)
    {
        ...
        $this->minify(
            (bool) env('output.minify.html', false),
            (array) env('output.minify.preserve.tags', ['TEXTAREA', 'CODE']),
            (array) env('output.minify.codeblock.buttons', [])
        );
    ...
    /**
     * Set template subfolder for controller templates views.
     * 
     * This sets the view subfolder paths within the `/resources/Views`, that controller templates 
     * should be resolved from instead of the root `Views` folder. 
     * It is useful for organizing templates into subdirectories based on features, sections, or modules.
     * 
     * Valid root includes:
     * - `/resources/Views/` - for MVC View directory.
     * - `/app/Modules/Views/` - for HMVC root view directory.
     * - `/app/Modules/<Module>/Views/` - for HMVC custom module view directory.
     *
     * @param string $pathname Subfolder pathname to look for template views.
     * 
     ...
     * ```
     */
    public final function setFolder(string $pathname): self
    {
        $this->subfolder = trim($pathname, TRIM_DS);

        // If template is already set, resolve it again to apply new folder path.
        if($this->template){
     ...
     * @see self::setFolder() for setting view subfolder paths.
     *
     * > **Note:** 
     ...
     *
     * Controls whether template content is minified and which HTML tags should
     * have their contents preserved during the minification process. 
     *
     * Code block actions can be enabled through the buttons configuration.
     *  -  If `null`, uses the default from `env(output.minify.codeblock.buttons)`.
     *  - If an empty array is provided,  disables all default buttons.
     *
     * @param bool $minifiable Whether to minify template content.
     ...
     * @param string[]|null $codeBlockButtons Code block actions to enable (e.g. `copy`, `ai`, `run`).
     *
     * @return self Returns the current template view instance.
     */
    ...
    ): self
    {
        ...
        $preserveTags ??= ($this->minification['preserve'] ?? []);

        $this->minification = [
            'minifiable'    => $minifiable,
            'preserve'      => $preserveTags,
            'buttons'       => $codeBlockButtons,
        ];
     ...
     *
     * @param string|string[] $template A single template name or an array 
     *                                  of template names to ignore from caching.
     *
     * @return self Return instance of template view class.
     * 
     * @see self::cacheOnly()
     ...
     * When this is used instead of `cacheExclude` all other templates will not be cached except 
     * the listed templates here.
     *
     ...
     * @param array<string,mixed> $options Optional options to pass to the `$onRenew` callback argument.
     ...
     * methods (`render()`, `contents()`, `promise()`, `exists()` or `info()`) are called. 
     *
     * **Search Paths:**
     ...
     *
     * @return self Return instance of template view class.
     * @throws InvalidArgumentException If `$template` is an empty-string or `$type` is not a supported view type.
     *
     * @see self::render()
     * @see self::contents()
     * @see self::promise()
     ...
     *      - `STATUS_SILENCE` if failed, silently terminate without error page 
     *              allowing you to manually handle the state.
     ...
     * 
     * - `location`   Full path to the view file.
     * - `type`       Content type (e.g., html, json).
     * - `template`   The view filename (without extension).
     * - `engine`     Template engine in use (e.g., default(PHP), twig, smarty).
     * - `module`     HMVC module name (or `root` if not in custom context).
     * - `size`       File size in bytes (0 if missing).
     * - `timestamp`  Last modified time (UNIX timestamp).
     * - `modified`   Last modified datetime (`Y-m-d H:i:s`).
     * - `dirname`    Directory containing the file.
     * - `extension`  File extension (e.g., php, twig).
     ...
     *     extension: ?string,
     *     filename: ?string
     * } Return the value for that key (null if not found), Otherwise return metadata
     *
     * @example - Example:
     * ```php
    ...
    public final function info(?string $key = null): mixed
    {
        ...
        $metadata['module'] = Luminova::isHmvc() ? ($this->module ?: 'root') : null;
    ...
    private function defaultTemplate(?array $options): mixed
    {
        $tpl = function(object $self, ?array $options, string $_VIEW_TYPE, string $_VIEW_FILEPATH): mixed {
            Boot::set(Boot::TEMPLATE_CONTEXT, $self->__id());
            ...
            try{
                $isValidSignature = $self->__is(Boot::get(Boot::TEMPLATE_CONTEXT));
            } catch(Throwable){
                $isValidSignature = false;
            } finally {
                Boot::remove(Boot::TEMPLATE_CONTEXT, false);
            }
    ...
    private function parseOptions(array $options = []): array 
    {
        ...
        if(!isset($options['title'])){
            $options['title'] = $this->toTitle(true);
        }

        if(!isset($options['subtitle'])){
            $options['subtitle'] = $this->toTitle();
        }
