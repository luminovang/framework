...
final class Password 
{
	...
	public static function strength(string $password, int $min = 8): int 
	{
		...
		$score = (mb_strlen($password, '8bit') >= $min) ? 1 : 0;
		$score += self::isSecure($password) ? 1 : 0;
		$patterns = [
			'/\p{N}/u',           // Contains digits (all scripts)
			'/\p{Lu}/u',          // Contains uppercase letter (all scripts)
			'/\p{Ll}/u',          // Contains lowercase letter (all scripts)
			'/[^\p{L}\p{N}]/u',   // Contains special characters (not letter, not number)
		];
