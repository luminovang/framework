...
class JWT implements LazyObjectInterface
{
     ...
     * @param string|null $aud The audience claim for the JWT, typically the API endpoint (default: APP_URL . '/api').
     *
     * @return self Returns the singleton instance of the JWT class.
     * @throws EncryptionException If the provided $path is not readable or writable.
     * @link https://luminova.ng/docs/0.0.0/security/jwt
     */
    ...
    {
        try {
            return Token::decode($token, new Key(
                self::key($userId, 'sha256'), 
                $this->algo ?? 'HS256'
            ));
        } catch (Throwable $e) {
     ...
     * ```php
     * $isValid = $jwt->validate('Bearer my-token', 'user-id', function(bool $isPassed, \stdClass $payload): bool {
     *     if (!$isPassed) {
