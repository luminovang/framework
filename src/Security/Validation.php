...
use Luminova\Utility\{Uuid, Luhn, Validator};
 ...
 * Basic:
 * ```
 * nullable, nullbyte, required, alphanumeric, alphabet, numeric, boolean,
 ...
 * Comparison:
 * ```
 * match, contains, regex, equals, not_equal
 * ```
 *
 * Array:
 * ```
 ...
 *     'nullable'|'nullbyte'|'required'|'alphanumeric'|'alphabet'|'numeric'|'boolean'|
 *     'hexadecimal'|'string'|'array'|'json'|'url'|'phone'|'email'|'luhn'|
 *     'between'|'max'|'min'|'size'|'limit'|'length'|'fixed'|'luhn'|
 *     'maxlength'|'minlength'|'maxsize'|'minsize'|'maxlimit'|'minlimit'|
 *     'float'|'integer'|'decimal'|'binary'|'scheme'|'latitude'|'longitude'|
 *     'username'|'name'|'is_value'|'uuid'|'ip'|'path'|
 *     'match'|'contains'|'equals'|'not_equal'|
 *     'in_array'|'is_list'|'keys_exists'|
 *     'callback'|'default'
 *
...
final class Validation implements InputValidationInterface, LazyObjectInterface
{
    ...
    ): bool
    {
        ...
        if ($this->body === null) {
            throw new RuntimeException(sprintf(
                'Validation failed: no input data available. '
                . 'Provide data using %s::setBody(), %s::setRequest(), '
                . 'or pass a valid request instance.',
                self::class,
                self::class
            ));
        }
        ...
        foreach ($this->rules as $field => $rule) {
            ...
            foreach ($rules as $entry) {
                ...
                [$ruleName, $ruleParam, $error] = is_array($entry) 
                    ? $entry
                    : self::toRuleArguments($entry);
                ...
                if (!$isEmpty || in_array($ruleName, ['default', 'fallback'], true)) {
                    $arguments = $ruleParam 
                        ? self::toArguments($ruleParam) 
                        : null;
    ...
    public function getFields(string|int $field = 0): array
    {
        ...
        $field = $this->resolveErrorField($field);
     ...
     * {@inheritdoc}
     */
    public function removeError(string|int $field): bool
    {
        if($this->failures === []){
            return true;
        }
    ...
    /**
     * Resolve error filed name.
     *
     * @param string|integer $field
     * 
     * @return string|null
     */
    private function resolveErrorField(string|int $field): ?string
    {
        if($field === ''){
            return null;
        }
        ...
        $field = array_keys($this->failures)[$field] ?? null;
     ...
     */
    private static function isInArray(mixed $value, ?array $args): bool
    {
        if (!$args) {
            return true;
        }
    ...
    ): bool 
    {
        try{
            return match ($name) {
                ...
                'nullbyte' => is_string($value) 
                    && self::isContains($value, "\0", $arguments[0] ?? false),
                'contains' => is_string($value)
                    && self::isContains($value, ...self::fillDefaults($arguments, ['', false])),
                'between'  => self::isBetween($value, ...self::fillDefaults($arguments, [1, 100])),
                'phone'    => (is_string($value) || is_numeric($value))
                    && Validator::isPhone((string) $value, ...self::fillDefaults($arguments, [10, 15])),
                'uuid'     => is_string($value) 
                    && Uuid::isValid((string) $value, (int) ($arguments[0] ?? 4)),
                'email'    => is_string($value) 
                    && Validator::isEmail($value, ...self::fillDefaults($arguments, [[], false])),
                'url'      => is_string($value) 
                    && Validator::isUrl($value, ...self::fillDefaults($arguments, [false, true])), 
                'username' => $this->isValidUsername($field, $value, $arguments),
                'name'     => is_string($value) 
                    && self::isName($value, ...self::fillDefaults($arguments, [false, 2, 150])),
                'is_value'  => ($value === ($arguments[0] ?? null)),
                'equals'    => ($value === self::toValue($this->body[$arguments[0] ?? null])),
                'not_equal' => ($value !== self::toValue($this->body[$arguments[0] ?? null])),
                'is_list'   => self::isCommaSeparated($value, $arguments[0] ?? 1),
                'in_array'    => self::isInArray($value, $arguments),
                'key_exists'  => self::isAnyKeyExists($value, $arguments),
                'keys_exists' => self::isKeysExists($value, $arguments),
                'latlng'      => is_string($value) && Validator::isLatLng(...array_merge(
                    explode(',', (string) $value, 2), 
                    self::fillDefaults($arguments, [false, 6])
                )),
                'lat', 'latitude'   => is_string($value) && 
                    Validator::isLat((string) $value, ...self::fillDefaults($arguments, [false, 6])),
                'lng', 'longitude' => is_string($value) && 
                    Validator::isLng((string) $value, ...self::fillDefaults($arguments, [false, 6])),
                ...
                'match', 'regex' => is_string($value) && $arguments
     ...
     *
     * When `$allow` is:
     * - true  → returns true only if the needle exists in the haystack
     * - false → returns true only if the needle does NOT exist in the haystack
     *
     * @param string $haystack The string to search in.
     * @param string $needle The substring to search for.
     * @param bool $allow Inverts the match condition.
     *
     * @return bool True if the condition passes, false otherwise.
     */
    private static function isContains(string $haystack, string $needle, bool $allow = false): bool
    {
        return $allow
