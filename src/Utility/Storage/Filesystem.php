 ...
 */
namespace Luminova\Utility\Storage;
...
use function \Luminova\Funcs\{root, is_platform, get_mime};
...
class Filesystem
{
     ...
     *
     * Keys:
     *  - system: Path to the system framework codes.
     *  - plugins: Path to the system third-party plugins.
     *  - library: Path to the libraries and third-party modules.
     *  - services: Path to serialized shared services.
     *  - controllers: Path to application controllers.
     *  - modules: Path to HMVC modules.
     *  - app: Path to application files.
     *  - writeable: Path to writeable files.
     *  - logs: Path to error logs.
     *  - caches: Path to application caches.
     *  - public: Path to public document root.
     *  - assets: Path to public assets directory.
     *  - views: Path to view templates.
     *  - routes: Path to application routes.
     *  - languages: Path to language modules.
     *
    ...
    ): bool
    {
        ...
        try {
            ...
            while (!feof($resource)) {
                ...
                if ($data === false) {
                    throw new FileException("Error reading from stream in file: {$filename}.");
                }
    ...
    ): bool 
    {
        ...
        } elseif(is_file($content) && is_readable($content)) {
            $typeOf = 'file';
    ...
    /**
     * Get a normalized path from the internal path list and return it from the
     * application root. 
     * 
     * This replaces the old getCompatible method.
     * 
     * * Keys:
     *  - system: Path to the system framework codes.
     *  - plugins: Path to the system third-party plugins.
     ...
     *  - languages: Path to language modules.
     *
     * @param string $name The path key-name (e.g, `app`).
     *
     * @return string Return the normalized path or an empty string if not found.
     */
     ...
     * Deprecated. Use path().
     *
     * @param string $name Path key.
     *
     * @return string Normalized path.
     * @deprecated
     * @ignore
     */
    public static function getCompatible(string $name): string 
    {
