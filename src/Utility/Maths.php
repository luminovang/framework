...
final class Maths
{
     ...
     * @var array $units
     */
    private static array $units = ['B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'];
    ...
    */
     ...
     *
     * This method automatically converts bytes to the most appropriate unit
     * (B, KB, MB, GB, TB, PB, EB, ZB, YB) and can optionally append the unit name.
     * Handles negative values and very large numbers safely.
     *
     * @param float|int $bytes The byte value to convert.
     * @param bool $addName Whether to append the unit name (Default: false).
     * @param int $decimals Number of decimal places to include (Default: 2).
     * 
     * @return string Return the formatted value, optionally including the unit.
     */
    public static function toUnit(float|int $bytes, bool $addName = false, int $decimals = 2): string
    {
        $negative = $bytes < 0;
        $bytes = abs($bytes);
        ...
        $formatted = ($negative ? '-' : '') . number_format($bytes, $decimals);
