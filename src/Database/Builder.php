...
use \Luminova\Database\Helpers\BuilderTrait;
...
{  
     ...
     *
     * > You may override the connection using `connection()` if needed.
     *
     * @example Basic usage
     * ```php
     ...
     *
     * @param array{name:array{comparison:string,value:mixed}}[] $columns The columns conditions 
     *      (from {@see self::column(...)} or raw array).
     ...
     * @param float[]|int[] $values An array of range boundaries. Must contain an even number of values.
     ...
     * 
     * @see self::notBetween() - Opposite behavior (values outside the range).
     * 
     * @example - Examples:
     * ```php
     * use Luminova\Database\Builder;
     ...
     *
     * @param string[] $columns An array of column names to include in the full-text match.
     ...
     *
     * @param array{column:array{comparison:string,value:mixed}}[] $conditions The conditions to group.
     ...
     *
     * @param array{column:array{comparison:string,value:mixed}}[] $firstGroup An array of first group conditions.
     * @param array{column:array{comparison:string,value:mixed}}[] $secondGroup An array of second group conditions.
     ...
     * @param (Closure(Builder $static):mixed)|mixed $value The value to compare against.
     *
     * @return array{name:array{comparison:string,value:mixed}} Returns an array representing a column structure.
     *
    ...
    public function bind(string $placeholder, mixed $value): self 
    {
        ...
        if ($placeholder === '' || !str_starts_with($placeholder, ':')) {
            throw new InvalidArgumentException(sprintf(
                'Invalid placeholder "%s". Must start with colon ":" (e.g. ":id", "%s")',
                $placeholder,
    ...
    /**
     * Remove a previously bound placeholder.
     *
     * If the placeholder does not exist, this method does nothing.
     *
     * @param string $placeholder Named placeholder (e.g. `:id`).
     * 
     * @return self Returns the current query builder instance.
     * @methodGroup QueryCondition Bind named placeholder to a value.
     */
    public function unbind(string $placeholder): self
    {
        unset($this->options['binds'][$placeholder]);
     ...
     *
     * @param array{column:array{comparison:string,value:mixed}}[] $firstGroup The first group conditions.
     ...
     * @param array{column:array{comparison:string,value:mixed}}[] $secondGroup The second group conditions.
     * @param string $connector Logical operator to join with previous conditions (`AND` or `OR`).
     * 
     * @return self Returns the current query builder instance.
     * 
    ...
    public function get(int $fetchMode = FETCH_OBJ, ?int $returnMode = null): mixed
    {
        ...
        $assert = $this->selector['assert'] ?? null;

        if($assert !== null){
            $this->assertStrictConditions((string) $assert, true);
        }
     ...
     *
     * @param string[] $columns List of columns to select for copying (defaults: `['*']`).
     * 
     * @return self Returns the current Builder instance.
     * @see self::bind() To bind named placeholder in SELECT column expression.
     * 
