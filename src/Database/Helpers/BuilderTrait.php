...
trait BuilderTrait 
{
     ...
     * 
     * @return array{0:?string,1:?string,2:?string} Return the value as upper-cased.
    ...
    private function addRawWhereClause(string &$sql): void 
    {
        ...
        if ($this->conditions === [] && !$this->findOuterWhere($sql)) {
            $query = preg_replace('/^\s*(AND|OR|WHERE)\b\s*/i', '', $query);
            $sql .= ' WHERE';
        }
    ...
    */
     ...
     *
     * @return array{0:string,1:bool,2:bool} First item is the SQL string, second is a boolean whether ORDER BY exists.
     */
     ...
     * @param int $index The index to append to the placeholder names.
     * @param string $operator The type of logical operator to use between 
     *      conditions within the group (default: 'OR').
