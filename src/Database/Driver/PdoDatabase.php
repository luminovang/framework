...
final class PdoDatabase implements DatabaseInterface 
{
     ...
     * Result fetch modes.
     * 
     * @var array<int,string> FETCH_MODES
     */
    ...
    {
        $this->assertStatement();
        $fetchMode = self::FETCH_MODES[$mode] ?? PDO::FETCH_OBJ;
        ...
        if($type === RETURN_NEXT){
            $result = ($mode === FETCH_CLASS || $mode === FETCH_OBJ) 
                ? $this->fetchObject(stdClass::class)
