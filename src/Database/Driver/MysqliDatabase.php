...
final class MysqliDatabase implements DatabaseInterface 
{
     ...
     * Result fetch modes.
     * 
     * @var array<int,mixed> FETCH_MODES
     */
    ...
    public function getConfig(string $property): mixed
    {
        ...
        if(in_array($property, ['username', 'password', 'port', 'host'], true)){
            return null;
        }
    ...
    {
       return self::getTypeFromValue($value, true);
    }
    ...
    public function execute(?array $params = null): bool 
    {
        ...
        try {
            ...
            $rowCount = (int) ($this->isSelect ? $this->stmt->num_rows : $this->stmt->affected_rows);
    ...
    public function getResult(int $returnMode = RETURN_ALL, int $fetchMode = FETCH_OBJ): mixed 
    {
        return match ($returnMode) {
            ...
            default => $this->fetch($returnMode, $fetchMode),
        };
    }
    ...
    public function fetch(int $returnMode = RETURN_ALL, int $fetchMode = FETCH_OBJ): mixed 
    {
        ...
        $withMode = self::FETCH_MODES[$fetchMode] ?? null;
        ...
        if($returnMode === RETURN_NEXT || $returnMode === RETURN_STREAM) {
            $method = ($withMode === FETCH_OBJ) ? 'fetch_object' : 'fetch_assoc'; 
        }

        $withMode = ($method === 'fetch_all') 
            ? (self::FETCH_MODES['mysqli'][$withMode] ?? MYSQLI_ASSOC)
            : null;
    ...
    private function getRow(mixed &$row): array 
    {
        ...
        if($isReference === null){
            return [self::getTypeFromValue($row), $row];
        }
        ...
        return [
            $row['type'] ?? self::getTypeFromValue($value), 
            $value
        ];
    }
     ...
     * @param bool $forInt Whether to return the integer constant for the type (true) or the character representation (false).
     * @return string|int The MySQLi parameter type as a character ('s', 'i', 'd', 'b') or an integer constant (PARAM_STR, PARAM_INT, etc.).
     */
    private static function getTypeFromValue(mixed $value, bool $forInt = false): string|int  
    {
       return match (true) {
            is_null($value) => $forInt ? PARAM_NULL : 's',
            is_float($value) => $forInt ? PARAM_FLOAT : 'd', 
            is_int($value), is_bool($value) => $forInt ? PARAM_BOOL : 'i',
            is_resource($value),
            (is_string($value) && (bool) preg_match('~[^\x09\x0A\x0D\x20-\x7E]~', $value))
                => $forInt ? PARAM_LOB : 'b',
