...
final class ViewCache
{
    ...
    */
     ...
     *  
     * @param DateTimeInterface|int|null $maxBurst Maximum burst time in seconds.
     * 
     * @return self Return class instance.
     */
     ...
     *  
     * @param string $url The request uri paths.
     * 
     * @return self Return class instance.
     */
     ...
     * 
     * @param string|null $version Optionally specify application version to retrieve (default: null).
     * 
     ...
     * 
     * This method return false if not expired, but if the view type is mismatched it return null.
     * 
     * @param string|null $type The type of cached content to check (e.g, `View::HTML`).
     * 
     * @return bool|null Return true if the cache is expired, otherwise false.
     */
    public function expired(?string $type = null): ?bool
    {
        if($type === '' || $type === '_'){
            return null;
        }
     ...
     * 
     * @param string|null $version Optionally specify application version to delete (default: null).
     * 
     ...
     * 
     * @param string|null $version Optionally specify application version to clear (default: null).
     * 
     ...
     * 
     * @return bool|null Return true if loading was successful, if miss-matched type return null, otherwise false.
     */
    ...
    ): ?bool
    {
        ...
        if($onBeforeRender === null){
            Boot::add(Boot::CLASS_METADATA, 'cache', true);
        }else{
            $onBeforeRender();
        }
    ...
    public function saveCache(string $content, array $headers = [], string $type = 'html'): bool
    {
        ...
        $length = (int) ($headers['Content-Length'] ?? strlen($content));
    ...
    private function preferLatestContent(string $pattern): bool
    {
        ...
        return preg_match('/^' . $pattern . '$/', (string) $this->uri) === 1;
    }
