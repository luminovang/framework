...
final class QueueTables
{
    ...
    public function createMysql(): int
    {
        return $this->db->exec("CREATE TABLE IF NOT EXISTS {$this->table} (
            ...
            KEY idx_date_retries (updated_at, retries)
    ...
    public function createOracle(): int
    {
        ...
        $this->db->exec("BEGIN
            ...
            EXECUTE IMMEDIATE 'CREATE INDEX idx_group_status ON {$this->table} (group_name, status)';
            EXECUTE IMMEDIATE 'CREATE INDEX idx_group_forever_status ON {$this->table} (group_name, forever, status)';
            EXECUTE IMMEDIATE 'CREATE INDEX idx_group_scheduled ON {$this->table} (group_name, scheduled_at)';
            EXECUTE IMMEDIATE 'CREATE INDEX idx_date_retries ON {$this->table} (updated_at, retries)';
    ...
    public function createMsAccess(): int
    {
        $result = $this->db->exec("CREATE TABLE {$this->table} (
            ...
            group_name TEXT(150),
            handler MEMO,
            arguments OLEOBJECT DEFAULT NULL,
            signature TEXT(32),
            outputs MEMO DEFAULT NULL,
     ...
     * @return void
     */
    private function createIndex(bool $isSqlite = false): void
    {
        $check = '';

        if($isSqlite){
            $check = 'INDEX IF NOT EXISTS ';
            $this->db->exec("CREATE UNIQUE INDEX IF NOT EXISTS uniq_task_group_signature 
                ON {$this->table} (group_name, signature)");
        }

        $this->db->exec("CREATE INDEX {$check}idx_group_status 
            ON {$this->table} (group_name, status)");
        $this->db->exec("CREATE INDEX {$check}idx_group_forever_status 
            ON {$this->table} (group_name, forever, status)");
        $this->db->exec("CREATE INDEX {$check}idx_group_scheduled 
            ON {$this->table} (group_name, scheduled_at)");
        $this->db->exec("CREATE INDEX {$check}idx_date_retries 
            ON {$this->table} (updated_at, retries)");
    }
}