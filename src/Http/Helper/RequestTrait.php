...
use Luminova\Http\{File, Header, Server, UserAgent};

/**
 * @mixin \Luminova\Http\Request
 */
trait RequestTrait
{
    /**
     ...
     * 
     * @var array<string,bool> BODY_METHODS
     */
    private const BODY_METHODS = [
        'POST'      => true,
        'PUT'       => true,
        'PATCH'     => true,
        'DELETE'    => true,
    ...
    /**
     * Lazily resolve and expose read-only properties.
     *
     * @param array{
     *  server: ?Header<\Luminova\Interface\LazyObjectInterface>,
     *  header: ?Server<\Luminova\Interface\LazyObjectInterface>,
     *  userAgent: ?UserAgent<\Luminova\Interface\LazyObjectInterface>
     * } $readonly 
     */
    ...
    /**
     * Proxy dynamic HTTP verb accessors (e.g. getPost, getPut).
     *
     * Resolves method calls like `$request->getPost('field', 'default')`
     * to values from the parsed request body.
     *
     * @param string $method Invoked method name.
     * @param array  $arguments [0 => field, 1 => default]
     *
     * @return mixed Return request field value or default. 
     * @throws InvalidArgumentException If method or field is invalid.
     * @internal
     */
    public function __call(string $method, array $arguments): mixed 
    {
        if($method === 'all'){
            return $this->getParsedBody((bool) $arguments[0] ?? false);
        }
        ...
        if (!$field || !is_string($field)) {
            throw new InvalidArgumentException(sprintf(
                'Method "%s()" requires a non-empty field name. Use %s::getParsedBody() instead.',
                $method,
                self::class
            ));
        }

        $httpMethod = ($method !== 'get' && str_starts_with($method, 'get')) ? substr($method, 3) : $method;
        $httpMethod = strtoupper($httpMethod);

        if(!in_array($httpMethod, Method::METHODS, true)) {
            throw new InvalidArgumentException(sprintf(
                'Method "%s()" is not a supported HTTP request accessor.', 
                $method
            ));
        }

        return $this->parsedBody[$httpMethod][$field] 
            ?? $arguments[1] 
            ?? null;
    }

    /**
     * Lazily read-only properties getter proxy.
     *
     ...
     * 
     * @return Server|Header|UserAgent
     * @throws InvalidArgumentException If property is not defined.
     * @internal
     */
    public function __get(string $property): mixed
    {
        $property = ($property === 'agent') ? 'userAgent' : $property;

        if (!array_key_exists($property, $this->readonly)) {
            throw new InvalidArgumentException("Undefined property: {$property}");
        }

        return $this->fromReadonlyLazyProperty($property);
    }

    /**
     * Disallow dynamic property setting.
     *
     * @param string $property Property name.
     *
     * @return void
     * @throws LogicException If attempting to modify a read-only property.
     * @internal
     */
    public function __set(string $property, mixed $value): void
    {
        ...
        throw new LogicException(array_key_exists($property, $this->readonly) 
            ? "Cannot modify read-only property: {$property}"
            : "Undefined or inaccessible property: {$property}"
        );
    }
     ...
     * - userAgent: Browser UserAgent
     *
     * Values are initialized on first access and cached for subsequent calls.
     * Attempting to modify any read-only property will throw an exception.
     *
     * @param string $property Property name.
     * 
     * @return Server|Header|UserAgent|null
     */
    private function fromReadonlyLazyProperty(string $property): mixed
    {
        if ($this->readonly[$property] === null) {
            $this->readonly[$property] = match ($property) {
                'server' => $this->isAuto 
                    ? Server::fromGlobal()
                    : new Server($this->getServerParams()),
                'header' => new Header($this->isAuto ? null : $this->serverParams),
    ...
    public function isProxy(): bool
    {
        ...
        foreach (self::$proxyHeaders as $header) {
            if (($this->isAuto && isset($_SERVER[$header])) || isset($this->serverParams[$header])){
                return $proxy = true;
            }
        }
    ...
    {
        $boundary = '------LuminovaFormBoundary';
        $boundary .= Luminova::hash('xxh3', microtime(true), fallbackAlgo: 'md5');
    ...
    public function getBody(): StreamInterface
    {
        ...
        return $this->body = Stream::fromString($body, readonly: true);
    }
    ...
    public function isBodySupported(?string $method = null): bool
    {
        ...
        return isset(self::BODY_METHODS[$method]);
    }
    ...
    private function wasUrlRewrite(): bool
    {
        if ((int) $this->server->get('IIS_WasUrlRewritten', 0) === 1) {
            ...
            $this->header->remove('IIS-WasUrlRewritten');
            return true;
        }
    ...
    private function parseRequestUrl(): void
    {
        ...
        if (!empty($parts['scheme'])) {
            ...
            $this->serverParams['HTTPS'] = $https ? 'on' : 'off';
        ...
        if ($this->body && empty($this->serverParams['CONTENT_TYPE']) && $this->isBodySupported($method)) {
     ...
     * 
     * @return array Return URL parts.
     * @throws InvalidArgumentException If failed to parse URI.
     */
     ...
     * 
     * @return array Return an associative array where keys are lowercase header names 
     ...
     * @param bool $isFlat Flatten developer-friendly flat structure
    ...
    /**
     * Extract multiple file attributes from a normalized file array.
     *
     * @param array $file Normalized file array.
     * @param string[] $keys List of file attribute keys to extract
     *                       (e.g. `name`, `tmp_name`, `size`, `type`, `error`).
     * @param int|null $index Optional index for multi-file inputs.
     *
     * @return array<string,mixed> Associative array of resolved file attributes.
     */
    private static function fileEntries(array $file, array $keys, ?int $index): array
    {
        $entries = [];
    ...
    ): ?UploadedFileInterface
    {
        ...
        $entries = self::fileEntries(
            $file,
            ['name', 'tmp_name', 'size', 'type', 'error', 'content', 'full_path'],
            $isFlat ? null : $index
        );

        $name    = $entries['name'];
        $temp    = $entries['tmp_name'];
        $size    = (int) $entries['size'];
        $type    = $entries['type'];
        $error   = (int) ($entries['error'] ?? UPLOAD_ERR_OK);
        $content = $entries['content'];
        $path    = $entries['full_path']; // For JS Upload Libraries
        ...
        $name ??= ($path && $path !== 'blob') ? $path : bin2hex(random_bytes(8));
