...
final class IP
{
    ...
    public static function info(?string $ip = null, array $metadata = []): ?object
    {
        ...
        if (file_exists($filename) && ($response = Filesystem::contents($filename)) !== false) {
        ...
        try {
            $response = Async::await(fn() => Luminova::kernel('http.client', false)
            ...
            if(make_dir($path)){
                Filesystem::write($filename, json_encode($options, JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT));
            }
    ...
    /**
     * Detect if an IP is a VPN, Tor exit node, hosting provider, or anonymized proxy.
     *
     * Uses the GeoIP2 Anonymous IP database from MaxMind. Requires the `geoip2/geoip2` package.
     *
     * @param string|null $ip IP address to check. Defaults to the current visitor IP.
     * @param string|null $mmdb Path to the GeoIP2 Anonymous IP database (default: `/writeable/caches/geoip2/anonymous-ip.mmdb`).
     *
     * @return bool Returns true if the IP is considered a VPN/proxy/anonymous, false otherwise.
     * @throws RuntimeException If the GeoIP2 Reader class is not installed.
     * 
     * > **Note:** 
     * > You must download the free GeoLite2 Anonymous IP database from:
     * > https://dev.maxmind.com/geoip/geoip2/geolite2/, https://github.com/maxmind/MaxMind-DB/tree/main/test-data
     * > and provide its path to the `$mmdb` parameter or place it in the default cache folder.
     */
    public static function isVpn(?string $ip = null, ?string $mmdb = null): bool
    {
        if (!class_exists(Reader::class)) {
            throw new RuntimeException(
                'GeoIp2\Database\Reader is not installed. Run `composer require geoip2/geoip2`.'
            );
        }

        $ip ??= self::get(); 
        $mmdb ??= root('/writeable/caches/geoip2/', 'anonymous-ip.mmdb');

        if (!is_file($mmdb)) {
            throw new RuntimeException(
                "GeoIP2 database file not found at: {$mmdb}. " .
                "Download it from https://dev.maxmind.com/geoip/geoip2/geolite2/"
            );
        }

        try {
            $data = (new Reader($mmdb))->anonymousIp($ip);
    ...
    private static function fetchTorNodeList(int $expiration): string|bool
    {
        ...
        if (file_exists(self::$torExitPath) && (time() - filemtime(self::$torExitPath) < $expiration)) {
            return Filesystem::contents(self::$torExitPath);
        }

        $result = Filesystem::contents(self::$torExitNodeListUrl);

        if($result === false){
           return false;
        }

        return Filesystem::write(self::$torExitPath, $result);
    }
