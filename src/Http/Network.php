 ...
 * ResponseInterface: 
 * 
 * @method static ResponseInterface get(UriInterface|string $url, array $options = [])
 * @method static ResponseInterface put(UriInterface|string $url, array $options = [])
 * @method static ResponseInterface patch(UriInterface|string $url, array $options = [])
 * @method static ResponseInterface delete(UriInterface|string $url, array $options = [])
 * @method static ResponseInterface head(UriInterface|string $url, array $options = [])
 * @method static ResponseInterface options(UriInterface|string $url, array $options = [])
 *
 * PromiseInterface:
 * 
 * @method static PromiseInterface getAsync(UriInterface|string $url, array $options = [])
 * @method static PromiseInterface postAsync(UriInterface|string $url, array $options = [])
 * @method static PromiseInterface putAsync(UriInterface|string $url, array $options = [])
 * @method static PromiseInterface patchAsync(UriInterface|string $url, array $options = [])
 * @method static PromiseInterface deleteAsync(UriInterface|string $url, array $options = [])
 * @method static PromiseInterface headAsync(UriInterface|string $url, array $options = [])
 * @method static PromiseInterface optionsAsync(UriInterface|string $url, array $options = [])
 */
final class Network
{
    /**
     ...
     * @var int SKIP_HEADER
     */
    public final const SKIP_HEADER = 5319;
     ...
     *
     * @return ResponseInterface|PromiseInterface|mixed Response object or a Promise for async requests.
     *
     ...
     * @throws \Luminova\Exceptions\Http\ConnectException If a connection to the server cannot be established.
     * @throws \Luminova\Exceptions\Http\ClientException If the client encounters a 4xx HTTP error.
     * @throws \Luminova\Exceptions\Http\ServerException If the server encounters a 5xx HTTP error.
     * 
     ...
     *
     * @param UriInterface|string $uri The target URI.
     * @param array<string,mixed> $options Client-specific request options.
     *
     * @return ResponseInterface Returns the request response object.
     *
     * @throws RequestException  On request errors.
     * @throws \Luminova\Exceptions\Http\ConnectException  On connection failures.
     * @throws \Luminova\Exceptions\Http\ClientException   On 4xx client errors.
     * @throws \Luminova\Exceptions\Http\ServerException   On 5xx server errors.
     */
     ...
     *
     * @param UriInterface|string $uri Request URI object or URI string.
     * @param string $method HTTP method (GET, POST, etc.).
     * @param array<string,mixed> $options Client-specific request options.
     *
     * @return PromiseInterface Promise resolving to the HTTP response.
     *
     * @example - Example:
     * ```php
     * Network::fetch(
     ...
     * @param array<string,mixed> $options Client-specific request options.
     *
     * @return PromiseInterface Promise resolving to the request response.
     */
    ...
    private static function create(): ClientInterface
    {
        ...
        $client = Luminova::kernel('http.client', false, self::$config);
     ...
     * @param bool $promise
     * 
     * @return PromiseInterface|ResponseInterface Return guzzle promise or response object for Novio client.
    */
     ...
     * @param bool $promise
     *
     * @return PromiseInterface|ResponseInterface|never Returns the response if present, or null if no response is available.
    ...
    ): PromiseInterface|ResponseInterface
    {
        ...
        self::handleLuminovaException($e);
    }
     ...
     * @throws \Exception
     */
    private static function handleLuminovaException(Throwable $e): never
    {
        ...
        if ($isStatic) {
            ...
            if ($url === '' && empty($base)) {
                $error = new BadMethodCallException(sprintf(
                    'Bad static method call: Network::%s(...) requires at least a URL, 
                    or set base_uri via Network::config().',
                    $method
                ));
