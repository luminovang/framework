...
use \Throwable;
use Luminova\Http\Uploader;
use Luminova\Http\Message\Stream;
use \Psr\Http\Message\StreamInterface;
use Luminova\Components\Media\ImageInfo;
use \Psr\Http\Message\UploadedFileInterface;
use Luminova\Http\Config\UploadedFileConfig;
use Luminova\Utility\{Converter, Mime, Encoder, Validator};
...
class File implements UploadedFileInterface, LazyObjectInterface, Arrayable
{
     ...
     */
    public final const UPLOAD_ERR_NO_SIZE = 9;
     ...
     */
    public final const UPLOAD_ERR_MIN_SIZE = 10;
     ...
     */
    public final const UPLOAD_ERR_NO_FILE_DATA = 11;
     ...
     */
    public final const UPLOAD_ERR_SKIPPED = 12;
     ...
     */
    public final const IF_EXIST_RETAIN = 'retain';
     ...
     */
    public final const IF_EXIST_RENAME = 'rename';
     ...
     */
    public final const IF_EXIST_OVERWRITE = 'overwrite';
     ...
     */
    public final const IF_EXIST_SKIP = 'skip';
     ...
     *
     * @var UploadedFileConfig|null $config
     */
    protected ?UploadedFileConfig $config = null;
    ...
    /**
     * Cached imageInfo object (if it's an image).
     *
     ...
     * @param Stream<StreamInterface>|string|null $content The file's content in string format, typically used when the file data is stored directly in memory as an alternative to using the `temp`.
    ...
    /**
     * Create a new uploaded file instance from an associative array.
     *
     * This method builds a file object using a structured array,
     * typically coming from a normalized $_FILES entry or a custom
     * file payload (e.g., API upload, blob upload, or internal mapping).
     *
     * Supported array keys:
     *
     * - index (int)          : File index in a multi-upload context. Default 0.
     * - name (string|null)   : Original file name.
     * - type (string|null)   : MIME type of the file.
     * - size (int)           : File size in bytes. Default 0.
     * - extension (string|null) : File extension (without dot).
     * - temp (string|null)   : Temporary file path on the server.
     * - error (int)          : PHP upload error code. Default UPLOAD_ERR_NO_FILE.
     * - content (StreamInterface|string|null): Raw file content (used when not uploaded via HTTP).
     * - isBlob (bool)        : Whether the file was created from binary/blob data.
     *
     * Missing keys are automatically filled with safe default values.
     *
     * @param array<string,mixed> $file File data map.
     *
     * @return static Returns a new uploaded file instance.
     * @throws InvalidArgumentException If neither `temp` nor `content` is provided.
     */
    ...
    public function getStream(): StreamInterface
    {
        ...
        if ($this->temp) {
            ...
            $this->content = new Stream($resource);
        ...
        return $this->content = Stream::fromString($body, readonly: true);
    }
    ...
    /**
     * Gets the image information of the file if it's an image.
     *
     * @return ImageInfo|null Return an ImageInfo object containing the dimensions and metadata of the image, 
     *          or null if the file is not an image or dimensions cannot be determined.
     */
    public function getImageInfo(): ?ImageInfo
    {
        $source = $this->temp ?? $this->getContent() ?? null;
        ...
        try {
            return $this->imageInfo ??= ImageInfo::from($source);
        } catch (Throwable) {
            return null;
        } finally {
            $source = null;
     ...
     *
     * @return string|null return the detected MIME type (e.g., "image/png"), 
     ...
     *
     * @return UploadedFileConfig Returns instance of upload configuration.
     * 
     * @example - Example:
     * 
     * ```php
     ...
     * ```
     */
    public function getConfig(): UploadedFileConfig
    {
        if($this->config instanceof UploadedFileConfig){
            return $this->config;
        }

        return $this->config = new UploadedFileConfig();
    }
    ...
    public function isBinary(): bool
    {
        ...
        return $this->isBin = ($this->content === null) 
            ? false 
            : Validator::isBinary($this->getContent());
    }
    ...
    public function isBase64Encoded(): bool
    {
        ...
        return $this->isBase64 = ($this->content === null) 
            ? false 
            : Encoder::isBase64Encoded(
                $this->getContent(), 
                (!$this->config instanceof UploadedFileConfig) 
                    ? false 
                    : ($this->config->base64StrictDecode ?? false)
            );
    }
     ...
     * can be provided as a `UploadedFileConfig` instance or an associative array.
     *
     * @param UploadedFileConfig|array<string,mixed> $config Upload configuration options.
     *
     * @return self Return instance of file object.
     * @throws RuntimeException If the upload path is invalid or points to a file.
     * @see UploadedFileConfig
     *
     ...
     * ]);
     * ```
     * @example Using a Uploaded File eConfig object
     * 
     * ```php
     * use Luminova\Http\Config\UploadedFileConfig;
     * 
     * $config = UploadedFileConfig::fromArray([
     ...
     * use Luminova\Http\Config\UploadedFileConfig;
     *
     * $config = new UploadedFileConfig(
     *     uploadPath:      '/writeable/uploads/',
     *     maxSize:         5_000_000,
     *     allowedTypes:    ['jpg', 'png'],
     *     base64StrictDecode:    true
     * );
     *
     ...
     * ```
     */
    public function setConfig(UploadedFileConfig|array $config): self
    {
        $this->config = ($config instanceof UploadedFileConfig) 
            ? $config 
            : UploadedFileConfig::fromArray($config);

        if($this->config->base64StrictDecode !== null){
            $this->isBase64 = null;
        }
     ...
     * 
     * Commonly used by the `Luminova\Http\Uploader` class to provide
     ...
     *      temp:?string,
     *      error:int,
     *      content:StreamInterface|string|null,
     *      isBlob:bool,
     *      isStream:bool
     * } Returns an array of file upload.
     */
    ...
    public function valid(): bool
    {
        ...
        if ($this->config instanceof UploadedFileConfig && !$this->isPassedCustomValidation()) {
            return false;
        }
    ...
    private function getErrorDetails(int $code): string
    {
        ...
        if ($code === UPLOAD_ERR_INI_SIZE || $code === UPLOAD_ERR_FORM_SIZE) {
            $maxUpload = Converter::toBytes(ini_get('upload_max_filesize'));
            $maxPost = Converter::toBytes(ini_get('post_max_size'));
        }
        
        $message = match($code) {
            UPLOAD_ERR_INI_SIZE => sprintf(
                'File exceeds server size limit (%s). ',
                Converter::toUnit($maxUpload, withName: true)
            ),
            UPLOAD_ERR_FORM_SIZE => sprintf(
                'File exceeds form size limit (%s). ',
                Converter::toUnit(min($maxUpload, $maxPost), withName: true)
            ),
        ...
        $troubleshoot = match($code) {
            UPLOAD_ERR_INI_SIZE, UPLOAD_ERR_FORM_SIZE => sprintf(
                ...
                Converter::toUnit($maxUpload, withName: true),
                Converter::toUnit($maxPost, withName: true)
            ),
    ...
    private function isPassedCustomValidation(): bool 
    {
        if ($this->config->maxSize !== null && $this->size > $this->config->maxSize) {
            ...
            $this->message = sprintf(
                ...
                Converter::toUnit($this->size, withName: true),
                Converter::toUnit($this->config->maxSize, withName: true)
            );
            return false;
        }
        ...
        if ($this->config->minSize !== null && $this->size < $this->config->minSize) {
            ...
            $this->message = sprintf(
                ...
                Converter::toUnit($this->size, withName: true),
                Converter::toUnit($this->config->minSize, withName: true)
            );
            return false;
        }
