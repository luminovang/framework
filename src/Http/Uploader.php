...
/**
 * Luminova Framework HTTP File uploader
 *
 * @package Luminova
...
final class Uploader
{
     ...
     * @param string|null $path Optional directory path for the upload. 
     *              If not provided, it falls back to the file configuration.
     ...
     * @param float|int $delay Optional delay between chunk writes in second (default: 0).
    ...
    ): bool|int
    {
        ...
        $error = true;
        $delay = ($delay === 0 && $chunk > 10_000_000) ? 0.01 : $delay;
        ...
        if ($file->getTemp() !== null) {
            ...
            $content = $file->isBase64Encoded() 
                ? base64_decode($file->getContent(), $config->base64StrictDecode) 
                : $file->getContent();
    ...
    ): ?string
    {
        ...
        if (!is_writable($path)) {
            $file->setMessage("Upload directory is not writable: " . $path);
            return null;
        }
        ...
        if(is_file($destination)){
            if($config->ifExisted === File::IF_EXIST_OVERWRITE){
                ...
                rename($destination, $path . 'backup_' . bin2hex(random_bytes(8)) . '_' . $filename);
            }elseif($config->ifExisted === File::IF_EXIST_RETAIN){
                $filename = 'copy_' . bin2hex(random_bytes(8)) . '_' . $filename;
                $destination = $path . $filename;
            }else{
                $file->setMessage(
