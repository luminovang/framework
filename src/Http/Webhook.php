...
class Webhook implements LazyObjectInterface
{
     ...
     * 
     * @return self Return instance of webhook class.
     */
    ...
    {
        $this->incoming['accept.event.name'] = $name;
        return $this;
    }
    ...
    public function isSignature(?array $data = null): bool
    {
        ...
        if($accepts){
            ...
            if(!$isInternal){
                $name = $this->incoming['accept.event.name'] ?? '_event';
    ...
    public function send(?string $field = null): ResponseInterface
    {
        ...
        return Luminova::kernel('http.client', false)
            ->request($this->method, $this->url, $options);
    }
