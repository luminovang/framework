...
trait LazyDynamicTrait
{
    ...
    {
        $this->onHydrateLazyObject(__FUNCTION__);
        $isSelf = $this->isLazySelf();

        return [
            'isSelf'    => $isSelf,
            'object'    => $isSelf ? $this : $this->lazyObject, 
            'namespace' => $this->lazyClassNamespace,
            'state'     => $this->lazyObjectState,
            'args'      => $this->lazyArguments,
    ...
    {
        $this->lazyClassNamespace = $data['namespace'] ?? null;
        $this->lazyObjectState = $data['state'] ?? 0;
        $this->lazyArguments = $data['args'] ?? null;
        $this->lazyInitializer = $data['initializer'] ?? null;

        if(!($data['isSelf'] ?? false)){
            $this->lazyObject = $data['object'] ?? null;
        }
    }
     ...
     * 
     * @return object<T>|LazyObjectInterface<T>|null Return lazy-loaded instance of the specified class or null.
