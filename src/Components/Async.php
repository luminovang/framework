...
final class Async
{
    ...
    {
        $stats = self::isFiber(false);

        if(!$stats && !$task instanceof Fiber){
            $start = microtime(true);
            $result = $task();

            if ($timeout > 0 && (microtime(true) - $start) > $timeout) {
                throw new RuntimeException("Task exceeded {$timeout} seconds.");
            }
    ...
    {
        $isFiber = self::isFiber(false);

        if(!$isFiber && !$task instanceof Fiber){
            return new Promise(function (callable $resolve, callable $reject) use ($task, $maxWait): void {
                $start = microtime(true);
     ...
     * @throws RuntimeException If PHP Fiber is not supported.
     */
    private static function isFiber(bool $throw = true): bool
    {
