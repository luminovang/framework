...
class Async
{
     ...
     *
     * @param Fiber|(callable():mixed) $task The task to await (e.g, `Fiber` or `callable`).
     * 
     ...
     *
     * @param (callable(mixed $result, string $id):void)|null $callback Optional callback function to be called after each task completes.
    ...
    private static function isFiber(): void
    {
        ...
        if (!self::$isFiberSupported) {
            throw new RuntimeException(
                ...
                'Consider using the `Luminova\Components\Queue` class as an alternative.'
            );
        }
    }
}