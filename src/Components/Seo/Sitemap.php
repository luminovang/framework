...
final class Sitemap
{
    ...
    ): bool  
    {
        ...
        Luminova::setExecutionTimeLimit(self::$config->maxExecutionTime);

        self::$memoryThreshold = round(Math::toBytes(ini_get('memory_limit')) * 0.6);
    ...
    private static function isLimitExceeded(): bool
    {
        ...
        if (memory_get_usage(true) >= self::$memoryThreshold) {
            self::_print(
                sprintf('Memory usage exceeded limit: %s', Math::toUnit(self::$memoryThreshold, withName: true)),
                'error',
                verbose: 'DEBUG'
            );
            self::_print('', verbose: 'DEBUG');
