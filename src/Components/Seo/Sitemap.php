...
final class Sitemap
{
    ...
    ): bool  
    {
        ...
        Env::setExecutionTimeLimit(self::$config->maxExecutionTime);

        self::$memoryThreshold = round(Converter::toBytes(ini_get('memory_limit')) * 0.6);
    ...
    private static function isLimitExceeded(): bool
    {
        ...
        if (memory_get_usage(true) >= self::$memoryThreshold) {
            self::_print(
                sprintf('Memory usage exceeded limit: %s', Converter::toUnit(self::$memoryThreshold, withName: true)),
                'error',
                verbose: 'DEBUG'
            );
            self::_print('', verbose: 'DEBUG');
