<?php
/**
 * Luminova Framework Mailer class.
 *
 * @package Luminova
...
use Luminova\Exceptions\{LuminovaException, MailerException};
...
final class Mailer implements LazyObjectInterface
{
    ...
    {
        $interface ??= Luminova::kernel('mailer', true, !PRODUCTION);

        if(is_string($interface)) {
            if(!class_exists($interface)) {
                MailerException::rethrow('no_client', $interface);
            }
        ...
        if (!$interface instanceof MailerInterface) {
            MailerException::rethrow('invalid_client', $interface::class);
        }
    ...
    public function send(MailTemplate|string|null $message = null): bool
    {
        ...
        try{
            ...
            $e = ($e instanceof LuminovaException) ? $e : new MailerException($e->getMessage(), $e->getCode(), $e);
    ...
    private function initialize(): void
    {
        ...
        $this->from = env('smtp.email.sender', 'hello@' . APP_HOSTNAME);
