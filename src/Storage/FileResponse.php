...
use Luminova\Exceptions\{FileException, RuntimeException, EncryptionException};
...
final class FileResponse
{
    ...
    ): bool
    {
        ...
        $now = Time::now($payload['tz'])->getTimestamp();
    ...
    private function perseHeaders(string $basename, int $expiry, array &$headers): string|bool
    {
        ...
        $headers['Content-Length'] ??= $filesize ?? ((int) filesize($filename));
