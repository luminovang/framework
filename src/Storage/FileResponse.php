...
final class FileResponse
{
     ...
     */
    public function sendImage(string $basename, int $expiry = 0, array $options = [], array $headers = []): bool
    {
     ...
     * @throws RuntimeException Throws if an error occurred during file processing.
     */
    public function sendSigned(string $fileHash, array $headers = [], int $length = (1 << 21), int $delay = 0): bool
    {
        $data = Crypter::decrypt($fileHash);
        ...
        return $this->send($payload['file'], ($expiresAt - $now), $headers, $length, $delay);
    }
     ...
     */
    public function sign(string $basename, int $expiry = 3600, DateTimeZone|string|null $timezone = null): string|bool
    {
        ...
        return Crypter::encrypt(json_encode($payload));
    }
    ...
    public function output(
        ...
        return $this->send($basename, $expiry, $headers, $length, $delay);
    }
    ...
    public function outputImage(
        ...
        return $this->sendImage($basename, $expiry, $options, $headers);
    }

    /**
     * @deprecated Use sendSigned() instead.
     */
    public function temporal(string $fileHash, array $headers = []): bool
    {
        return $this->sendSigned($fileHash, $headers);
    }
    ...
    {
        return $this->sign($basename, $expiry);
    }
