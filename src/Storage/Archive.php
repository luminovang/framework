...
class Archive 
{
    ...
    private static function create(string $tarFile, string $source, string $pattern = ''): void
    {
        try{
            ...
            throw new FileException(
                $e->getMessage(), 
                ($e->getCode() === 0) ? ErrorCode::FILESYSTEM_ERROR : $e->getCode(), 
                $e
            );
        }
    }
     ...
     * @param string $zipFolder The folder path inside the zip.
    ...
    ): array
    {
        $relativePath = $zipFolder 
            ? $zipFolder 
            : $zipFolder . DIRECTORY_SEPARATOR . basename($filepath);

        if (!self::ignore($ignores, $relativePath)) {
    ...
    ): array 
    {
        ...
        foreach ($iterator as $item) {
            ...
            $relativePath = rtrim($zipFolder, TRIM_DS) . DIRECTORY_SEPARATOR . $name;
            ...
            if ($item->isDir()) {
                self::$zip->addEmptyDir($relativePath);
