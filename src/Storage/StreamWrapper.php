<?php
/**
 * Luminova Framework stream wrapper.
 *
 * @package Luminova
...
use \Luminova\Exceptions\{ErrorCode, RuntimeException};
...
class StreamWrapper 
{
    ...
    */
     ...
     * @throws RuntimeException Throws if no resource is found.
     */
    ...
    protected function call(string $fn, mixed ...$arguments): mixed 
    {
        if(!$this->isResource()){
            throw new RuntimeException(
                "Operation '{$fn}' is not allowed because the resource is invalid or unavailable.",
     ...
     */
    protected function setExternalContext(string $path): array|bool
    {
        if (preg_match('#^(?<scheme>[^:]+)://(?<fullpath>.+)$#', $path, $matches)) {
            return $matches;
        } 
