...
class Filesystem implements Stringable
{
    ...
    /**
     * Streams a file resource to output.
     *
     * Large files are read in chunks to reduce memory usage.
     * Text or binary handling can be forced, or automatically detected.
     *
     * @param resource $stream An open file stream.
     * @param int|null $filesize Optional Optional known file size in bytes, to determine handler (default: `null`).
     * @param bool|string|null $mime Optional MIME type used to detect contents.
     * @param int $length Chunk size in bytes (default: 2MB).
     * @param int $delay Optional delay between chunks in microseconds.
     *
     * @return bool Returns true if data was streamed successfully.
     * > The file handler is always closed after reading.
     */
    ...
    ): bool
    {
        ...
        if (!$isLargeFile) {
            $result = fpassthru($stream) > 0;
        }else{
            $isText = false;
            $mime ??= MIME::guess($stream) ?? 'application/octet-stream';
            ...
            $result = $isText
                ? self::readFromText($stream, $filesize, $length, $delay)
                : self::readFromBinary($stream, $length, $delay);
        }
    ...
    public static function write(string $filename, mixed $content, int $flags = 0, $context = null): bool
    {
        ...
        if (self::isResource($content, 'stream')) {
            return self::writeFromStream($filename, $content, $flags, $context);
        }
    ...
    /**
     * @deprecated Method is deprecated use writeFromStream() instead.
     */
    ...
    {
        return self::writeFromStream($filename, $resource, $flags, $context, $delay);
    }
    ...
    ): bool
    {
        ...
        try {
            ...
            // Rewind the stream if not at start
     ...
     */
    private static function readFromBinary(mixed $handler, int $length = (1 << 21), int $delay = 0): bool
    {
     ...
     * @return bool Return true if the file was successfully read, false otherwise.
     */
    private static function readFromText(
        mixed $handler, 
        ?int $filesize = null, 
        int $length = (1 << 21), 
        int $delay = 0
