...
class Filesystem implements Stringable
{
     ...
     * @param float|int $delay The delay between each chunk in seconds (default:, 0 second).
     * 
     ...
     * @param float|int $delay Delay in seconds between retries (default 0.5ms).
    ...
    ): bool
    {
        ...
        try {
            ...
            $mode .= (Validator::isBinary($content) ? 'b' : '');
    ...
    public static function mkdir(string $path, int $permissions = 0777, bool $recursive = true): bool 
    {
        ...
        try {
            Luminova::permission('rw', $path, false);
        } catch (Throwable $e) {
            $error .= ' ' . $e->getMessage();
        }
    ...
    public static function symbolic(string $target, string $link): bool
    {
        ...
        if (is_platform('windows')) {
            ...
            if ($result !== 0) {
                return self::symError('Windows symlink creation failed');
            }
    ...
    /**
     * Return the last-modified Unix timestamp for the given source.
     *
     * Accepts a `Stream` object, a PHP resource, a filesystem path string, or
     * any other `StreamInterface` implementation. Returns `0` when the
     * modification time cannot be determined.
     *
     * @param SplFileObject|StreamInterface|Stream|string $source The source to inspect.
     * 
     * @return int Return a Unix timestamp of source mtime, or `0` if can't be determined.
     */
    public static function getLastModified(mixed $source): int
    {
        try{
            if ($source instanceof Stream) {
                return (int) ($source->stat('mtime') ?? 0);
            }

            if ($source instanceof SplFileObject) {
                return (int) ($source->fstat()['mtime'] ?? 0);
            }
            ...
            if ($source instanceof StreamInterface) {
                ...
                if (is_string($uri) && is_file($uri)) {
                    ...
                    return (int) @filemtime($uri);
                }
            }

            if (is_resource($source)) {
                $stat = fstat($source);
                return ($stat !== false) ? (int) ($stat['mtime'] ?? 0) : 0;
            }

            if (is_string($source) && is_file($source)) {
                clearstatcache(true, $source);
                return (int) @filemtime($source);
            }
        }catch(Throwable){}
     ...
     * @param resource|null $context Optional stream context created via stream_context_create().
     * @param float|int $delay Optional delay between writing chunks in seconds (default: 0).
     * @param int &$bytes The number of bytes written, or 0 on error.
     *
    ...
    ): bool
    {
        ...
        try {
            ...
            $mode = (($flags & FILE_APPEND) !== 0) ? 'a' : 'w';
            $mode .= (str_contains($meta['mode'] ?? '', 'b') || Validator::isBinary($resource) ? 'b' : '');
     ...
     * @param int $length           Maximum chunk size in bytes (default: 2MB).
     * @param float|int $delay      Delay in seconds between chunk reads (default: 0).
    ...
    ): bool 
    {
        ...
        if (is_file($origin)) {
            ...
            if (!self::mkdir(dirname($dest), Security::$dirPermissions)) {
                return false;
            }
        ...
        if (!self::mkdir($dest, Security::$dirPermissions)) {
            return false;
        }
