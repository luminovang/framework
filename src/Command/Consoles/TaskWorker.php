...
class TaskWorker extends Console 
{
    ...
    public function run(?array $params = null): int
    {
        ...
        try {
            $runCommand = match($name){
                ...
                'task:tail', 'task:listen'     => $this->taskTail(),
    ...
    /**
     * Create and return the task queue instance.
     *
     * @param bool $isRunner Whether the caller is the background runner.
     *
     * @return Queue|null The task queue instance, or null if it cannot be created.
     */
    private function getTaskInstance(bool $isRunner = false): ?Queue
    {
        $error = static function (string $message) use ($isRunner): null {
            if ($isRunner) {
        ...
        $class = '\\' . ltrim(
            $this->input->getAnyOption('class', 'c', 'App\\Tasks\\TaskQueue'),
            '\\'
        );

        if (!class_exists($class)) {
            $normalized = '\\App\\Tasks\\' . ltrim($class, '\\');

            if (!class_exists($normalized)) {
                return $error(sprintf(
                    'Invalid or missing task class: [%s]. Cannot proceed.',
                    $class
                ));
            }
        ...
        if (!$task instanceof Queue) {
            return $error(sprintf(
                'Class [%s] must extend %s to manage tasks.',
                $class,
                Queue::class
            ));
        }
        ...
        $task->setCommandInput($this->input);
    ...
    private function taskList(): int 
    {
        ...
        foreach ($result['tasks'] as $item) {
            $class = $this->task->getClosure($item['handler'] ?? '');

            if($class){
                $handler = "{$class}@anonymous";
            }else{
    ...
    private function taskInfo(): int
    {
        ...
        foreach ($row as $key => $value) {
            ...
            if($key === 'handler'){
                $class = $this->task->getClosure($value ?? '');

                if($class){
                    $value = "{$class}@anonymous";
                }
            }
     ...
     *      -c=App\\Tasks\\Test \
     *      --inotify (optional, requires inotify extension) \
     *      -ts=5 (optional, timeout seconds for inotify) \
     *      -tm=0 (optional, timeout microseconds for inotify)
     */
    private function taskTail(): int
    {
        $this->task = $this->getTaskInstance(true);
        ...
        if (!$info) {
            Terminal::writeln(sprintf(
                'Missing logging flag. Set "%s::$eventLogging = true".',
                $taskNamespace
            ), 'red');
            return STATUS_ERROR;
        }
        ...
        $filename = $info[1];
        $isInotify = $this->input->hasOption('inotify');
        $this->task = null;
        ...
        try{
            $tail = new FileTail(
                ...
                grep: $this->input->getAnyOption('grep', 'g', null),
                since: $this->input->getAnyOption('since', 's', null),
                exclude: $this->input->getAnyOption('exclude', 'x', null)
            );
        } catch(RuntimeException $e){
            Terminal::error("{$e->getMessage()}  Run command without --inotify option.");
        } catch(Throwable $e){
            Terminal::error("Task events log error: {$e->getMessage()}");
            return STATUS_ERROR;
        }

        $tail->onError(function(string $message, string $status): void {
            Terminal::writeln("Task log error: {$message}", 'red');
        })->onRotate(function(string $filename): void {
            Terminal::writeln("Log file rotated. Reopening...", 'yellow');
        })->onOpen(function(string $filename, string $mode) use ($taskNamespace): void {
            Terminal::writeln(sprintf(
                "Listening (%s): %s",
                $mode,
                $taskNamespace
            ));
            Terminal::writeln("File: {$filename}");
            Terminal::writeln('Press Ctrl+C to stop.', 'green');
            Terminal::newLine();
            Terminal::spinner();
        })->onOutput(function(string $line){
            Terminal::writeln($line);
        });

        $result = $tail->run(
            (int) $this->input->getAnyOption('wait', 'w', 200_000),
            (int) $this->input->getAnyOption('retry', 'r', 0)
        );

        if(!$result || $tail->hasError()){
            Terminal::error($tail->getError() ?: 'Unknown error occurred while listening to task events.');
            return STATUS_ERROR;
        }
    ...
    private function getSignalPath(Queue $task): array
    {
        ...
        $path = root("/writeable/logs/queues/signal/");
     ...
     * 
     * @return string Return column title
     */
