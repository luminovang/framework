...
{
    /**
     * Custom known command options.
     * 
     * @var array $knownOptions
     */
    private array $knownOptions = [];

    /**
     * System known command options.
     * 
     * @var array $systemKnown
     */
    private array $systemKnown = [
        'h', 'help', 'no-header', 'no-color',
    ];
     ...
     * @param string[] $knownOptions Optional known command options/flags,
     *          use during {@see self::hasUnknownOption()} validation.
     * 
     * @example - Example:
     * ```php
     * $input = new Input([
     ...
     * ```
     */
    public function __construct(private array $command, array $knownOptions = [])
    {
        if($knownOptions !== []){
            $this->setKnownOptions($knownOptions);
        }
    }
    ...
    /**
     * Set the known command options.
     * 
     * Each entry may contain multiple aliases separated by commas (e.g. "-h, --help").
     * These are split, trimmed, and stripped of leading dashes ("-" or "--"),
     * then stored as keys in an associative array.
     *
     * @param string[] $options Option names (e.g, `verbose`).
     * 
     * @return self Returns instance of command input.
     * @see self::hasUnknownOption() To check if command contains unknown options.
     */
    public function setKnownOptions(array $options): self
    {
        $this->knownOptions = [];
    ...
    public function hasOption(string $option, ?string $alias = null): bool
    {
        ...
        return $alias !== null 
            && array_key_exists($alias, $options);
    }

    /**
     * Check if the command contains any unknown option or flag.
     *
     * Returns true if at least one option from the command input does not
     * exist in the known options list.
     * 
     * @param string|null &$unknown The found unknown option parsed by reference.
     *
     * @return bool True if an unknown option is found, false otherwise.
     * @see self::setKnownOptions() To set unknown options.
     */
    public function hasUnknownOption(?string &$unknown = null): bool
    {
        $unknown = null;

        if($this->knownOptions === []){
            return false;
        }
        ...
        foreach (array_keys($options) as $opt) {
            if(in_array($opt, $this->systemKnown)){
                continue;
            }

            if (!isset($this->knownOptions[$opt])) {
                $unknown = $opt;
                return true;
            }
        }
