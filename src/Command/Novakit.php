...
final class Novakit 
{
     ...
     * 
     * @var array<string,class-string>|null $consoles 
     */
     ...
     * @return void
     */
    public static function run(Input|array $input): void
    {
        Terminal::init();
    ...
    ): int
    {
        ...
        try{
            ...
            $input->setKnownOptions(array_keys($info['options'] ?? []));
     ...
     * @param string $mode The lookup mode: `system` for internal commands, 
     *          or `global` for custom/console commands.
     *
    ...
    public static function find(string $group, string $mode = 'global'): ?string 
    {
        ...
        $controller = match($novakit){
            ...
            'server', 'serve', 'http' => Server::class,
            default         => null
        };
    ...
    private static function build(string $group): array
    {
        ...
        try{
            $instance = self::getProperty(self::$newConsole);
            $pos = strpos($group, ':');
     ...
     * 
     * @return object Returns a reflection wrapper with a get() accessor method.
     */
    private static function getProperty(?Console $console): object
    {
        return new class($console) {
    ...
    private static function autoload(): void 
    {
        ...
        $commands = (static function (string $_): mixed {
            return include_once $_;
        })($bin);
    ...
    private static function failed(string $command): int 
    {
        ...
        $suggested = Commands::suggest($command);

        if($suggested){
            Terminal::writeln($suggested, stream: Terminal::STD_ERR);
        }
