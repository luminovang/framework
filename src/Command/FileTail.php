...
class FileTail
{
     ...
     */
    public function run(float|int $wait = 0.2, int $retry = 0): bool
    {
        $this->clearErrors();
     ...
     * 
     * @param float|int $seconds The number of seconds to sleep between entries.
     * 
     * @return bool Returns true on successful execution, false if an error occurs 
     *      (errors will be added to the error list).
     */
    protected function runPolling(float|int $seconds): bool
    {
        while (true) {
            if (!$this->check()) {
                uwait($seconds);
     ...
     *      (errors will be added to the error list).
     */
    protected function runInotify(float|int $seconds): bool
    {
