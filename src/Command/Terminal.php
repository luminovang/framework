...
final class Terminal
{
    ...
    {
        $user = self::shell(is_platform('windows') ? 'echo %USERNAME%' : 'whoami') 
            ?: get_current_user();
    ...
    public static function visibility(bool $visibility = true): bool
    {
        ...
        return self::shell($command) !== null;
    }
    ...
    */
    ...
    private static function getWindowsHiddenPassword(string $message, int $timeout): string
    {
        ...
        if (file_put_contents($vbscript, $inputBox) !== false) {
            $password = self::shell("cscript //nologo " . escapeshellarg($vbscript));
        }

        if (!$password) {
            $password = self::shell('powershell -Command "Read-Host -AsSecureString | ConvertFrom-SecureString"');
        }
