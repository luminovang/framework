...
use function Luminova\Funcs\{is_platform, get_temp_file, list_to_array};
...
final class Terminal
{
     ...
     *
     * @param float|int $seconds Number of seconds to pause execution (default: 10).
     ...
     * ```
     */
    public static function freeze(float|int $seconds = 1, bool $clear = true): void
    {
        if ($clear) {
            self::clear();
     ...
     * @param float|int $sleep Delay in seconds between frames to control animation speed (default: 0.1s).
    ...
    public static function parseCommands(array $arguments, bool $controller = false): array
    {
        ...
        if ($script === 'novakit' || $script === 'php' || preg_match('/^.*\.php$/', $script)) {
            ...
            $scriptCommend = $arguments;

            if ($hasSpace) {
                $scriptCommend = implode(' ', $arguments);
                $scriptCommend = $scriptCommend[0];
            }
            
            $result['command'] = $scriptCommend;
            $result['name'] = $arguments[1] ?? '';
        }
    ...
    public static function run(string $bin, array $args = [], array $options = []): Result
    {
        ...
        $validation = self::validateBinary($bin);

        if ($validation !== null) {
            [$exitCode, $error] = $validation;
            $hasError = true;
        } else {
        ...
        try {
            return self::handleProcessResult(
                $process,
                $pipes,
                $command,
                $args,
            ...
            self::cleanupProcess($process, $pipes);
        }
    }
    ...
    /**
     * Starts a command as a detached background process.
     *
     * Supports both Windows and Unix-like systems. On Linux/macOS, the process PID
     * is returned directly. On Windows, the PID is read from the optional PID pipe
     * written by the child process.
     *
     * Supported options:
     * - `cwd` (string): Working directory for the command.
     * - `pid-pipe` (string): PID file or Windows pipe path.
     * - `stdout` (?string): Redirect stdout to a file.
     * - `stderr` (?string): Redirect stderr to a file.
     *
     ...
     *
     * @return int|null The background process PID, or null on failure.
     */
    public static function async(string $command, array $options = []): ?int
    {
        $pipe = null;
        $previousCwd = null;
        ...
        $pidPipe = $options['pid-pipe'] ?? null;
        $stdout  = $options['stdout'] ?? $options['output'] ?? null;
        $stderr  = $options['stderr'] ?? $options['error'] ?? null;
        $isWindows = is_platform('windows');

        try {
            if ($isWindows && is_string($pidPipe)) {
                $pipe = @fopen($pidPipe, 'r+') ?: null;
            }

            $pid = $isWindows
                ? self::asyncWindows($command, $stdout, $stderr)
                : self::asyncLinux($command, $pidPipe, $stdout, $stderr);

            $pid = ($pid && $pid > 0) ? $pid : null;

            if ($pipe) {
                return self::readPidPipe($pipe) ?: $pid;
            }

            return $pid;
        } finally {
            if ($previousCwd !== false && $previousCwd !== null) {
                chdir($previousCwd);
            }
     ...
     * @param string|null $stdout         Option stdout redirect file (default: `null`).
    ...
    ): ?int 
    {
        ...
        $phpPath ??= (self::whichPhp() ?? PHP_BINARY);
        ...
        if(is_platform('windows')){
            $pidPipe = Terminal::createTempPipePath('luminova_async_run_legacy');

            if ($pidPipe !== null) {
                $arguments['pid-pipe'] = $pidPipe;
            }
        }

        $arguments = self::escapeArguments($arguments);
    ...
    /**
     * Creates a temporary IPC pipe path for command communication.
     *
     * Attempts to use a Windows named pipe first, then falls back to a writable
     * file-based pipe. The returned path is escaped for safe shell usage.
     *
     * @param string|null $namespace Optional pipe namespace prefix.
     *
     * @return string|null Escaped pipe path or null when no pipe is available.
     */
    public static function createTempPipePath(?string $namespace = null): ?string
    {
        $namespace ??= 'luminova_system_command_';
        ...
        $paths = [
            is_platform('windows')
                ? "\\\\.\\pipe\\{$namespace}{$id}"
                : null,
            root("/writeable/.pipe/{$namespace}", "{$id}.txt"),
        ];
    ...
    ): string
    {
        ...
        return self::$session['id'] = $prefix . Luminova::hash(
            $algo, 
            implode('|', $info), 
            $binary,
            fallbackAlgo: null
     ...
     */
    public static function readPidPipe(mixed $source, float $timeout = 2.0): ?int
    {
        if (is_string($source)) {
            ...
            if (!$source) {
                if (is_file($source)) {
                    return (int) trim(file_get_contents($source));
                }
    ...
    /**
     * Escapes command arguments for safe shell execution.
     *
     * Positional arguments are escaped as values. Associative arguments are
     * converted to `key=value` pairs before being escaped.
     *
     * Only scalar values are supported. A non-scalar value will throw an exception.
     *
     * @param array<int|string,mixed> $arguments Command arguments.
     *
     * @return array<int,string> Escaped shell arguments.
     *
     * @throws InvalidArgumentException If an argument value is not scalar.
     */
    public static function escapeArguments(array $arguments): array
    {
        $escaped = [];
        ...
        foreach ($arguments as $key => $value) {
            if ($value !== null && !is_scalar($value) && !is_array($value)) {
                throw new InvalidArgumentException(sprintf(
                    'Invalid command argument "%s", type "%s".',
                    $key,
                    gettype($value)
                ));
            }
    ...
    /**
     * Validate process binary path.
     *
     * @param string $bin
     * @return array|null
     */
    private static function validateBinary(string $bin): ?array
    {
        if (!self::isSafeBinaryPath($bin)) {
            return [
                113,
                "Unsafe binary path/name: '{$bin}'"
            ];
        }
     ...
     * @return Result
     */
    private static function handleProcessResult(
        mixed $process,
        array &$pipes,
        string $command,
        array $args,
    ...
    ): Result 
    {
        ...
        if ($throwOnError && !empty($hasError)) {
     ...
     * @return void
     */
    private static function cleanupProcess(mixed &$process, array &$pipes): void
    {
    ...
    /**
     * Starts a detached background process on Unix-like systems.
     *
     * The process runs independently of the current request. Standard output and
     * standard error may be redirected separately. If no output files are
     * specified, both streams are discarded.
     *
     * If a PID pipe is provided, the process ID is written to it; otherwise, the
     * PID is returned through the shell output.
     *
     * @param string $command Command to execute.
     * @param string|null $pidPipe Optional file to store the process ID.
     * @param string|null $stdout Optional file to receive standard output.
     * @param string|null $stderr Optional file to receive standard error.
     *
     * @return int|null The process ID, or null if the process could not be started.
     */
    private static function asyncLinux(
        string $command,
        ?string $pidPipe = null,
        ?string $stdout = null,
        ?string $stderr = null
    ): ?int
    {
        ...
        if ($stdout !== null) {
            $redirect .= ' 1> ' . escapeshellarg($stdout);
        }

        if ($stderr !== null) {
            $redirect .= ' 2> ' . escapeshellarg($stderr);
        }
        ...
        $pid = $pidPipe
            ? ' echo $! > ' . escapeshellarg($pidPipe)
            : ' echo $!';

        $cmd = sprintf('%s%s &%s', $command, $redirect, $pid);
    ...
    /**
     * Starts a detached background process on Windows.
     *
     * The process is launched using `start /B`. Standard output and standard error
     * may be redirected independently. If neither stream is redirected, the
     * process inherits the default console handles.
     *
     * @param string $command Command to execute.
     * @param string|null $stdout Optional file to receive standard output.
     * @param string|null $stderr Optional file to receive standard error.
     *
     * @return int Always returns 0. The child process is responsible for recording
     *             its own PID when needed.
     */
    private static function asyncWindows(
        string $command,
        ?string $stdout = null,
        ?string $stderr = null
    ): ?int
    ...
    */
        ...
        if ($continue !== 'OK') {
            ...
            if (!$invisible && self::visibility(false) === false) {
                $continue = self::prompt('
                    Your password may be visible while typing, do you wish to continue?', 
                    ['yes', 'no'], 
    ...
    private static function getWindowsHiddenPassword(string $message, int $timeout): string
    {
        ...
        $vbscript = get_temp_file('input_pwd', 'vbs');
        $inputBox = 'wscript.echo(InputBox("'. addslashes($message) . '", "", ""))';

        try{
            if (
                $vbscript !== null 
                && file_put_contents($vbscript, $inputBox) !== false
            ) {
            ...
            if ($password === '' || $password === null) {
                $password = self::exec(
                    'powershell -Command "Read-Host -AsSecureString | ConvertFrom-SecureString"',
                    true
                );
            }
