...
class Notification implements LazyObjectInterface
{
    ...
    private function message(string $type, Message $config, ?string $to = null): ?MessageCaster
    {
        try {
            ...
            $message = CloudMessage::new()
                ->withChangedTarget($target, $to)
                ->withNotification(self::create(
                    $config->getTitle(),
                    $config->getBody(),
                    $config->getImageUrl()
                ))->withDefaultSounds();
    ...
    public function send(Message|array $config, bool $validateOnly = false): self
    {
        ...
        throw new RuntimeException(
            "Invalid input: Expected a Message instance and must call method setToken 
            or an array with a 'topic' key token."
        );
    }
    ...
    public function channel(Message|array $config, bool $validateOnly = false): self
    {
        ...
        throw new RuntimeException(
            "Invalid input: Expected a Message instance 
            and must call method setTopic or an array with a 'topic' key included."
        );
    }
    ...
    public function condition(Message|array $config, bool $validateOnly = false): self
    {
        ...
        throw new RuntimeException(
            "Invalid input: Expected a Message instance and must call method setCondition 
            or an array with a 'conditions' key included."
        );
    }
