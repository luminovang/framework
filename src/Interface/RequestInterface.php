...
interface RequestInterface extends \Psr\Http\Message\RequestInterface
{
     ...
     * @param UriInterface|string|null $uri The request URI. Can be a `UriInterface` instance 
     *                  or a string containing path and query.
     * @param StreamInterface|array $body Request body. Can be a PSR-7 stream 
     *                  or an associative array (form data, JSON, etc.).
     * @param UploadedFileInterface[]|array<string,array> $files Uploaded files object 
     *                  or an associative array (e.g., `['image' => [...]]`).
     ...
     *
     * Query parameters are parsed from the URL query string (e.g., `/path?param=value`)
     * and merged with parameters from the HTTP `QUERY` method when available.
     *
     * If a parameter name is provided, its value is returned. If the name is null,
     * the full query string is returned as an RFC 3986 encoded string.
     *
     * @param string|null $name The query parameter name to retrieve. If null, returns the
     * encoded query string.
     * @param mixed $default The default value returned if the parameter does not exist.
     *
     * @return mixed Returns the parameter value, the default value when not found, or
     * the encoded query string when `$name` is null.
     *
    ...
    /**
     * Retrieves all query parameters from the request URL or QUERY request body.
     *
     * Query parameters are parsed from the URL query string and merged with parameters
     * provided in the HTTP `QUERY` request body when available. This allows query data
     * to be accessed consistently regardless of whether it is sent through the URL or
     * a `QUERY` method payload.
     *
     * When the same parameter exists in both sources, QUERY request body parameters
     * take precedence over URL query string values.
     *
     * @return array<string,mixed> Returns the merged query parameters as an associative
     * array.
     *
     * @see self::getQuery()
     */
