...
interface InputValidationInterface 
{
     ...
     *
     * Uses rules defined via `$this->rules`, {@see self::setRules()}, or {@see self::addField()}.
     ...
     *
     * @param RequestInterface|LazyObjectInterface|null $request Optional request instance used to resolve input data.
     * @param bool $stopOnFailure Whether to stop validation immediately after the first failure (default: `false`).
     *
     * @return bool Returns `true` when validation passes, otherwise `false`.
     *
     * @throws RuntimeException Thrown if:
     * - No input data is available for validation.
     * - Request object for LazyObjectInterface does not resolve to a RequestInterface
     ...
     *
     * @return void
     * @see static:removeError()
     */
    public function clearErrors(): void;

    /**
     * Remove a specific validation error details.
     *
     * @param string|integer $field Field name or index (default: 0 = first field with errors).
     * 
     * @return bool Return true if error was removed, otherwise false.
     */
     ...
     * @param string[] $reservedUsernames List of reserved usernames (case-insensitive) 
     *              (e.g. ['root', 'admin', 'system']).
     *
