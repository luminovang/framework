...
use Luminova\Utility\{Encoder, Converter};
...
final class Performance
{
     ...
     * @var int $endMemory
     */
    private static int $endMemory = 0;
    ...
    public static function stop(?string $style = null, ?array $command = null): void
    {
        ...
        $isApi = Luminova::isApiRequest(true);
        ...
        $classMetadata = Boot::get(Boot::CLASS_METADATA);
        $compression = Encoder::getLastResponseCompression();
        ...
        $info = [
            ...
            'Script Path'   => APP_CONTROLLER_INDEX,
            ...
            'Is Cache'     => (!empty($classMetadata['isCache'])) ? 'YES' : 'NO',
            'Is Static Cache' =>  (!empty($classMetadata['isStaticCache'])) ? 'YES' : 'NO',
        ];

        if($compression !== null){
            $info['Compression'] = sprintf(
                'Compressed: %s (%d → %d bytes)',
                $compression['encoding'] ?? 'none',
                $compression['contentLength'] ?? 0,
                $compression['compressedLength'] ?? 0
            );
        }

        if(!$isApi){
            $content = $_SERVER['HTTP_LMV_SENT_CONTENT_TYPE']
                ?? $_SERVER['CONTENT_TYPE'] 
                ?? $_SERVER['HTTP_CONTENT_TYPE']
                ?? '';

            if(!$content || str_contains($content, 'text/html')){
     ...
     * 
     * - `$iterations`: how much work you do per run
     * - `$samples`:    how many times you repeat that whole run
     *
     * @param string $name The benchmark identifier.
     * @param int $iterations Number of iterations to execute inside the callback.
     * @param (callable(int $n, bool $warmup):void) $callback Code under test. Receives $iterations 
     ...
     * 
     * $iterations = 10;
     * $logFile = root('/writeable/logs/', 'debug.log');
     *
     ...
     * });
     * 
     * $mono = Performance::benchmark('Monolog', $iterations, function ($n) use ($logFile) {
     *      $logger = new Logger('my_app_channel');
     *      $logger->pushHandler(
     *          new StreamHandler($logFile, Logger::DEBUG, true, 0644, false)
     *      );
     *      $logger->debug('Test Mono Speed.');
     * });
     * 
    ...
    /**
     * Executes a performance benchmark and returns timing and memory metrics.
     *
     * This method measures execution time and memory usage for a given callback.
     * It supports warm-up runs and multiple samples to reduce noise from CPU
     * scheduling, cache state, and runtime initialization effects.
     *
     * Concept:
     * - $iterations: number of operations performed per run
     * - $samples:    number of full benchmark runs (used for stability)
     * - $warmups:    initial runs ignored to stabilize runtime state
     *
     * The callback is executed inside an internal loop for each iteration.
     *
     * @param string $name Benchmark identifier.
     * @param int $iterations Number of iterations per sample run.
     * @param (callable(int $i, bool $warmup):void) $callback Code under test. Signature: function(int $i): void
     * @param int $samples Number of benchmark samples (between: 1-50).
     * @param int $warmups Number of warm-up runs before measurement (between: 0-10).
     *
     * @return array{
     *      name: string,
     *      iterations: int,
     ...
     *      avg_ms: int|float,
     *      memory_kb: int|float,
     *      loop_overhead_ns: int|float
     * } Benchmark results including timing and memory metrics.
     *
     * @see self::compare() To compare multiple benchmark results.
     *
     * @example - Example:
     * ```php
     * $iterations = 100000;
     *
     * $ulid = Performance::benchmark('ULID', $iterations, function (int $i, bool $warmup): void {
     *     \Luminova\Funcs\ulid();
     * });
     *
     * $uuid = Performance::benchmark('UUID', $iterations, function (int $i, bool $warmup): void {
     *     \Luminova\Funcs\uuid();
     * });
     *
     * print_r(Performance::compare($uuid, $ulid));
     * ```
     * 
     * @example - Example:
     * ```php
    ...
    ): array
    {
        ...
        $loopOverhead = hrtime(true) - $start;
        $times = [];
        $memory = [];
        ...
        for ($sample = 0; $sample < $samples; $sample++) {
            ...
            $times[] = max(0, $elapsed - $loopOverhead);
            $memory[] = $endMem - $startMem;
        }

        sort($times);

        $medianNs = $times[intdiv($samples, 2)];
        $totalMs = $medianNs / 1_000_000;
        ...
        return [
            ...
            'memory_kb' => array_sum($memory) / $samples / 1024,
            'loop_overhead_ns' => $loopOverhead,
        ];
    }

    /**
     * Compare multiple benchmark results and return a performance verdict.
     *
     * Determines which benchmark is fastest, differences in milliseconds,
     * execution ratios, and a human-readable verdict including percentage difference.
     * 
     * benchmarks:
     * - name: string,
     * - total_ms: int|float
     *
     * @param array<string,mixed> ...$benchmarks Benchmark result arrays from self::benchmark().
     *
     * @return array{
     *     fastest:string,
     *     verdict:string,
     ...
     *     speed_percent:array<string,float>,
     *     slower_percent:array<string,float>
     * } Returns comparison results among all benchmarks.
     * 
     ...
     * $iterations = 10;
     *
     * $t1 = Performance::benchmark('Test 1', $iterations, fn() => });
     * $t2 = Performance::benchmark('Test 2', $iterations, fn() => });
     * $t3 = Performance::benchmark('Test 3', $iterations, fn() => });
     * 
     * print_r(Performance::compare($t1, $t2, $t3));
     * ```
     */
    public static function compare(array ...$benchmarks): array
    {
        if (count($benchmarks) < 2) {
            throw new InvalidArgumentException('At least two benchmarks are required for comparison.');
        }

        $fastest = null;

        foreach ($benchmarks as $idx => &$benchmark) {
            $benchmark['name'] ??= "Test " . ($idx + 1);

            $total = (float) ($benchmark['total_ms'] ?? INF);

            if ($fastest === null || $total < $fastest['total_ms']) {
                $fastest = [
                    'name' => $benchmark['name'],
                    'total_ms' => $total
                ];
        ...
        $fastestTotal = max($fastest['total_ms'], 0.000001);

        $diffs       = [];
        $ratios      = [];   // how many times slower (1.0 = equal, 2.0 = twice slower)
        $speedScores = [];   // 0–100% speed relative to fastest
        $percentSlower = []; // how much slower than fastest (can exceed 100%)

        foreach ($benchmarks as $benchmark) {
            $name  = $benchmark['name'];
            $total = (float) ($benchmark['total_ms'] ?? 0);

            $safeTotal = max($total, 0.000001);

            // Absolute difference (ms)
            $diffs[$name] = round($total - $fastestTotal, 6);
            $ratios[$name] = round($safeTotal / $fastestTotal, 4);

            // % faster (0–100)
            $speedScores[$name] = round(
                (1 - ($fastestTotal / $safeTotal)) * 100,
                2
            );

            // % slower than fastest (can exceed 100%)
            $percentSlower[$name] = round(
                (($total - $fastestTotal) / $fastestTotal) * 100,
                2
            );
        }
        ...
        foreach ($benchmarks as $benchmark) {
            ...
            $speed = $speedScores[$name];
            $ratio = $ratios[$name];

            $verdicts[] = match (true) {
                $diffs[$name] == 0.0 =>
                    sprintf('Both %s and %s perform equally.', $fastestName, $name),

                $ratio >= 2 =>
                    sprintf(
                        '%s is significantly faster than %s (%.2fx faster, %s%% faster).',
                        $fastestName,
                        $name,
                        $ratio,
                        $speed
                ...
                default =>
                    sprintf(
                        '%s is slightly faster than %s (%.2fx faster, %s%% faster).',
                        $fastestName,
                        $name,
                        $ratio,
                        $speed
        ...
        return [
            'fastest'        => $fastestName,
            'verdict'        => implode(' ', $verdicts),
            'differences'    => $diffs,
            'ratios'         => $ratios,
            'speed_percent'  => $speedScores,
            'slower_percent' => $percentSlower,
        ];
    }
    ...
    public static function metrics(bool $html = true): string
    {
        ...
        $db = Boot::get(Boot::QUERY_PROFILING) ?? [];

        return sprintf(
            "Execution: %s%sDatabase: %s%sMemory: %s%sFiles: %d%s",
            Converter::toTimeUnit(abs(self::$endTime - self::$startTime) * 1_000, withName: true),
            $separator,
            Converter::toTimeUnit(
                ($db['global']['time'] ?? 0) * 1_000, 
                withName: true
            ),
            $separator,
            Converter::toUnit(self::$endMemory - self::$startMemory, withName: true),
            $separator,
            $total,
            $unused
        );
    ...
    public static function fileInfo(string $context = 'web'): array 
    {
        ...
        $filename = Boot::get(Boot::CLASS_METADATA)['filename'] ?? null;
        $index = 0;
        ...
        foreach (self::$filesLoaded ?? get_included_files() as $file) {
            ...
            $filtered = Luminova::toDisplayPath($file);
            $category = 'Others';
            $color = '#6f4f06';
    ...
    private static function logApiPerformanceMetrics(array $info): void 
    {
        ...
        $logData['database_executions'] = Boot::get(Boot::QUERY_PROFILING) ?? [];
    ...
    private static function showWebPerformanceMetrics(string|null $style, array $info): void 
    {
        ...
        if($db){
            ...
            $connections = (int) ($db['global']['connections'] ?? 0);
            ...
            foreach ($db['queries'] as $idx => $row) {
                ...
                $dbHtml .= '<td style="padding:4px 8px;border:1px solid #113535;">'. Converter::toTimeUnit($row['time'] * 1_000, withName: true) . ' ' . self::getVerdict($row['time']) . '</td>';
        ...
        $tabs = [
            ...
            'lmv-tab-database' => 'Database Executions <span ' . $counters . '>' . $queries . '/' . $connections. '</span>'
        ];
    ...
    {
        $controllers = max(Boot::get(Boot::CLASS_METADATA)['controllers'] ?? 0, 0);
