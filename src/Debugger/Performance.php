    ...
    private static function showWebPerformanceMetrics(string|null $style, array $info): void 
    {
        ...
        if($db){
            ...
            foreach ($db['queries'] as $idx => $row) {
                ...
                $dbHtml .= '<td style="padding:4px 8px;border:1px solid #113535;">'. Maths::toTimeUnit($row['time'] * 1_000, true) . ' ' . self::getVerdict($row['time']) . '</td>';
        ...
        $counters = 'style="background-color: #fff;color: #000;border-radius: 50%;padding: 2px;font-size: 13px;font-weight: bold;"';
