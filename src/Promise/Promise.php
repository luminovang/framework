...
final class Promise implements PromiseInterface
{
     ...
     * @see deferred() - It is useful for handling asynchronous 
     *      or deferred tasks where the result is not immediately available.
     *
     * @example Basic usage:
     * ```php
    ...
    /**
     * Run a callable asynchronously and return a Promise that resolves with its result.
     *
     * Wraps the given task in a Fiber and returns a `PromiseInterface` that:
     * - Resolves with the task result when the Fiber completes.
     * - Rejects if the task throws an exception or exceeds the specified timeout.
     *
     * @param callable():mixed $task The task to execute asynchronously.
     * @param int $timeout Maximum wait time in seconds (0 = unlimited).
     * @param float $delay Delay in seconds between Fiber ticks (default 0.1s).
     *
     * @return PromiseInterface Resolves to the task result or rejects with an exception.
     *
     * @example - Run an async network request:
     * ```php
     * use Luminova\Http\Network;
     *
     * Promise::await(fn() => Network::get('https://example.com'))
     *     ->then(fn($result) => print_r($result))
     *     ->catch(fn(Throwable $e) => echo 'Error: ' . $e->getMessage())
     *     ->finally(fn() => echo 'Done');
     * ```
     */
    public static function await(callable $task, int $timeout = 0, float $delay = 0.1): PromiseInterface
    {
        return new self(function (callable $resolve, callable $reject) use ($task, $delay, $timeout): void {
            $task = new Fiber($task);
                ...
                while (!$task->isTerminated()) {
                    ...
                    if ($timeout > 0 && (microtime(true) - $start) >= $timeout) {
                        $reject(new RuntimeException(
                            "Promise did not resolve within {$timeout} seconds."
                        ));
                        return;
                    }
     ...
     * @param (callable(mixed $input):void)|null $onResolved Callback to be executed 
     *          when the promise is resolved.
     * @param (callable(mixed $input):void)|null $onRejected Callback to be executed 
     *          when the promise is rejected.
     *
     * @return self Return instance of promise interface.
     */
     ...
     * @param (callable(mixed $input):mixed)|null $onResolved The callback to be executed 
     *      when the promise is resolved.
     * @param (callable(mixed $input):mixed)|null $onRejected The callback to be executed 
     *      when the promise is rejected.
     *
     * @return void
     */
        ...
        try {
            ...
            if ($result instanceof PromiseInterface) {
                if($result->is(self::PENDING)){
                    $result->then(
                        static fn($v) => $promise->resolve($v), 
                        static fn($e) => $promise->cancel($e)
                    );
                    return;
                }
     ...
     * @return void
     *
     * @throws ErrorException If an invalid state transition is attempted 
     *          or if the promise references itself.
     */
     ...
     *
     * @param (callable(mixed $resolve = null, ?callable $reject = null): void)|null $onResolved 
     * @param (callable(\Throwable $error): void)|null $onRejected
     *
     * @return void
