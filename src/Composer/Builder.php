...
{  
     ...
     * @param string|null $onComplete Optional green-coloured success message printed after the task.
     *
     * @return void
     */
    ...
    private static function upgrade(): void 
    {
        ...
        $cmd = sprintf(
            '%s %s install --no-dev --optimize-autoloader',
            strtoupper(substr(PHP_OS, 0, 3)) === 'WIN'
                ? 'set LM_DEBUG_MODE=1 &&'
                : 'LM_DEBUG_MODE=1',
            Updater::whichComposer()
        );
    ...
    private static function onComplete(array $result, $destination, bool $isImport = false) : void 
    {
        ...
        if (!$isImport && $isSystem) {
            ...
            if ($cwd) {
                ...
                $cmd = sprintf(
                    'cd %s && %s %s dump-autoload --optimize --no-dev',
