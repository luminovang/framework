...
class Updater
{
     ...
     * Composer context
     * 
     * @var bool isForUpdate
     */
    private static bool $isForUpdate = false;
     ...
     */
    private static function delete(string $file, string $ctx = ''): bool 
    {
    ...
    private static function checkAndMoveFolderRecursive(string $source, string $destination): void
    {
        ...
        foreach ($iterator as $item) {
            ...
            $sysName = self::displayPath($dstPath);

            if (self::isUpdater($sysName)) {
                self::$selfInfo = [
            ...
            if(self::doRename($srcPath, $dstPath) && self::$isForUpdate){
                self::$updates++;
                Terminal::writeln('  - Updating  ' . Terminal::color($sysName, 'green'));
            }
        }
    }
