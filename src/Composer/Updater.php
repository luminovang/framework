...
class Updater
{
    ...
    private static function checkAndMoveFolderRecursive(string $source, string $destination): void
    {
        ...
        foreach ($iterator as $item) {
            ...
            if(self::doRename($srcPath, $dstPath) && self::$isForUpdate){
                Terminal::writeln("[{$sysName}] Updated.");
            }
        }
    }
    ...
    private static function finalizeInstall(string $source): void
    {
        ...
        if ($code !== 0) {
            Terminal::newLine();
            Terminal::error(
                'Composer autoload optimization failed. Your installation files were restored where possible.'
            );
        ...
        $version = Luminova::VERSION ?? '3.7.3';
