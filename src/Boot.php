...
final class Boot
{
    ...
    public static function warmup(): void
    {
        ...
        self::tryIncludeModule('autoload.php', 'plugins', false);
        // self::tryIncludeModule('Config/Env.php', 'system');
        self::tryIncludeModule('constants.php', 'bootstrap', false);

        self::config();
        self::override();

        self::tryIncludeModule('functions.php', 'bootstrap');
        self::tryIncludeModule('funcs/string.php', 'bootstrap');
        self::tryIncludeModule('funcs/array.php', 'bootstrap');
    ...
    public static final function profiling(string $mode, ?array $command = null): void
    {
        ...
        if (!env('debug.show.performance.profiling', false)) {
            return;
        }
    ...
    /**
     * Check if VCI is auto autoload resolver.
     *
    ...
    ): mixed
    {
        if (preg_match('/^([a-z]+):(.+)$/i', $path, $match)) {
            ...
            $path = match ($scheme) {
                'package' => self::resolve($match[2], 'plugins'),
                default   => in_array($scheme, Luminova::$systemPaths, true)
                    ? root($scheme, $match[2])
                    : $path,
            };
        }
    ...
    public static function registerAliases(bool $autoload = true, int &$registered = 0): bool
    {
        ...
        if (env('feature.app.class.alias', false)) {
            $modules ??= self::tryIncludeModule('Config/Modules.php', 'app', false, true);
        }
    ...
    private static function setAppDefaults(): void
    {
        ...
        $limit = env('script.execution.limit');

        if($limit !== null){
            Env::setExecutionTimeLimit((int) $limit);
        }

        // Set default timezone
        $tz = (string)env('app.timezone');
        ...
        $enc = (string) env('app.mb.encoding');
        ...
        $abort = env('script.ignore.abort');
        
        if($abort !== null){
            Env::setIgnoreUserAbort((bool) $abort);
        }
    }
