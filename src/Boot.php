...
final class Boot
{
    ...
    public static function resolve(string $file, string $from = 'system'): ?string
    {
        if(
            ...
            && isset(self::$config['luminova.paths'][$from])
        ){
            $root = self::toPath(self::$config['luminova.paths'][$from]);
        } else{
    ...
    {
        $isProduction ??= defined('PRODUCTION') ? (PRODUCTION !== false) : true;
    ...
    private static function trySharedModules(): void 
    {
        ...
        if(empty(self::$config['share.namespaces'] ?? null)){
            self::$config['share.namespaces'] = [
                'Luminova\\Funcs\\'  => self::toPath(self::$config['luminova.paths']['bootstrap']),
                'Luminova\\'         => self::toPath(self::$config['luminova.paths']['system']),
            ];
    ...
    ): mixed
    {
        if($shared && self::$config){
            ...
            if (!isset($paths[$from])) {
                self::onError(
                    sprintf('Missing "%s" path in luminova.paths configuration.', $from)
                );
            }

            $root = self::toPath($paths[$from]);
        } else{
            $root = match($from){
                'system'    => __DIR__ . '/',
                'app'       => __DIR__ . '/../app/',
