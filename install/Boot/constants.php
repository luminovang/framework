 ...
 *
 * The function outputs the given values using the available tracer
 * implementation or falls back to var_dump(). Execution continues
 * after dumping.
 *
 ...
 *
 * All active output buffers are cleared before dumping to prevent
 * previously buffered output from being sent before the debug output.
 *
 * @param mixed ...$vars One or more values to dump.
 *
 * @return never Always exit 1.
 * @see dump()
 */
 ...
 * @param string|float|int $value The value to set for the environment variable.
 ...
 * ```
 */
function setenv(string $key, string|float|int $value, bool $persist = false): bool
{
    $key = trim($key);
